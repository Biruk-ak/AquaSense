 ...
 * Unit tests: HighUseBillingCycle3
 * @copyright Biruk-ak
 */
...
import { HighUseBillingCycle3Service } from './high-use-billing-cycle3.service';
...
describe('HighUseBillingCycle3Service', () => {
  let service: HighUseBillingCycle3Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new HighUseBillingCycle3Invoice(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        HighUseBillingCycle3Service,
        { provide: getRepositoryToken(HighUseBillingCycle3Invoice), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(HighUseBillingCycle3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'HighUseBillingCycle3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'HighUseBillingCycle3.updated' }));
  });
