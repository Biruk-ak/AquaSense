 ...
 * Unit tests: StandardBillingCycle4
 * @copyright Biruk-ak
 */
...
import { StandardBillingCycle4Service } from './standard-billing-cycle4.service';
...
describe('StandardBillingCycle4Service', () => {
  let service: StandardBillingCycle4Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new StandardBillingCycle4Invoice(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        StandardBillingCycle4Service,
        { provide: getRepositoryToken(StandardBillingCycle4Invoice), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(StandardBillingCycle4Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'StandardBillingCycle4.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'StandardBillingCycle4.updated' }));
  });
