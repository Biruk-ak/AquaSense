 ...
 * Unit tests: IndustrialBillingCycle4
 * @copyright Biruk-ak
 */
...
import { IndustrialBillingCycle4Service } from './industrial-billing-cycle4.service';
...
describe('IndustrialBillingCycle4Service', () => {
  let service: IndustrialBillingCycle4Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new IndustrialBillingCycle4Invoice(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        IndustrialBillingCycle4Service,
        { provide: getRepositoryToken(IndustrialBillingCycle4Invoice), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(IndustrialBillingCycle4Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'IndustrialBillingCycle4.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'IndustrialBillingCycle4.updated' }));
  });
