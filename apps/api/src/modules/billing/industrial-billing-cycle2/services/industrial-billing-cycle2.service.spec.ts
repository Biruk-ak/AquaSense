 ...
 * Unit tests: IndustrialBillingCycle2
 * @copyright Biruk-ak
 */
...
import { IndustrialBillingCycle2Service } from './industrial-billing-cycle2.service';
...
describe('IndustrialBillingCycle2Service', () => {
  let service: IndustrialBillingCycle2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new IndustrialBillingCycle2Invoice(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        IndustrialBillingCycle2Service,
        { provide: getRepositoryToken(IndustrialBillingCycle2Invoice), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(IndustrialBillingCycle2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'IndustrialBillingCycle2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'IndustrialBillingCycle2.updated' }));
  });
