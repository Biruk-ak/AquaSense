 ...
 * Unit tests: IndustrialBillingCycle5
 * @copyright Biruk-ak
 */
...
import { IndustrialBillingCycle5Service } from './industrial-billing-cycle5.service';
...
describe('IndustrialBillingCycle5Service', () => {
  let service: IndustrialBillingCycle5Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new IndustrialBillingCycle5Invoice(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        IndustrialBillingCycle5Service,
        { provide: getRepositoryToken(IndustrialBillingCycle5Invoice), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(IndustrialBillingCycle5Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'IndustrialBillingCycle5.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'IndustrialBillingCycle5.updated' }));
  });
