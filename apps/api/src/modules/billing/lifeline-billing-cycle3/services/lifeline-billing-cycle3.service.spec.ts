 ...
 * Unit tests: LifelineBillingCycle3
 * @copyright Biruk-ak
 */
...
import { LifelineBillingCycle3Service } from './lifeline-billing-cycle3.service';
...
describe('LifelineBillingCycle3Service', () => {
  let service: LifelineBillingCycle3Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new LifelineBillingCycle3Invoice(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        LifelineBillingCycle3Service,
        { provide: getRepositoryToken(LifelineBillingCycle3Invoice), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(LifelineBillingCycle3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LifelineBillingCycle3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LifelineBillingCycle3.updated' }));
  });
