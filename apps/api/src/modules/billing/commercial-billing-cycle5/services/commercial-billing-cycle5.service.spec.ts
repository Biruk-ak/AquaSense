 ...
 * Unit tests: CommercialBillingCycle5
 * @copyright Biruk-ak
 */
...
import { CommercialBillingCycle5Service } from './commercial-billing-cycle5.service';
...
describe('CommercialBillingCycle5Service', () => {
  let service: CommercialBillingCycle5Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new CommercialBillingCycle5Invoice(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        CommercialBillingCycle5Service,
        { provide: getRepositoryToken(CommercialBillingCycle5Invoice), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(CommercialBillingCycle5Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CommercialBillingCycle5.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CommercialBillingCycle5.updated' }));
  });
