 ...
 * Unit tests: LifelineBillingCycle2
 * @copyright Biruk-ak
 */
...
import { LifelineBillingCycle2Service } from './lifeline-billing-cycle2.service';
...
describe('LifelineBillingCycle2Service', () => {
  let service: LifelineBillingCycle2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new LifelineBillingCycle2Invoice(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        LifelineBillingCycle2Service,
        { provide: getRepositoryToken(LifelineBillingCycle2Invoice), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(LifelineBillingCycle2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LifelineBillingCycle2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LifelineBillingCycle2.updated' }));
  });
