 ...
 * Unit tests: HighUseBillingCycle1
 * @copyright Biruk-ak
 */
...
import { HighUseBillingCycle1Service } from './high-use-billing-cycle1.service';
...
describe('HighUseBillingCycle1Service', () => {
  let service: HighUseBillingCycle1Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new HighUseBillingCycle1Invoice(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        HighUseBillingCycle1Service,
        { provide: getRepositoryToken(HighUseBillingCycle1Invoice), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(HighUseBillingCycle1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'HighUseBillingCycle1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'HighUseBillingCycle1.updated' }));
  });
