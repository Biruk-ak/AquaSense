 ...
 * Unit tests: LifelineBillingCycle4
 * @copyright Biruk-ak
 */
...
import { LifelineBillingCycle4Service } from './lifeline-billing-cycle4.service';
...
describe('LifelineBillingCycle4Service', () => {
  let service: LifelineBillingCycle4Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new LifelineBillingCycle4Invoice(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        LifelineBillingCycle4Service,
        { provide: getRepositoryToken(LifelineBillingCycle4Invoice), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(LifelineBillingCycle4Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LifelineBillingCycle4.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LifelineBillingCycle4.updated' }));
  });
