 ...
 * Unit tests: CommercialBillingCycle2
 * @copyright Biruk-ak
 */
...
import { CommercialBillingCycle2Service } from './commercial-billing-cycle2.service';
...
describe('CommercialBillingCycle2Service', () => {
  let service: CommercialBillingCycle2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new CommercialBillingCycle2Invoice(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        CommercialBillingCycle2Service,
        { provide: getRepositoryToken(CommercialBillingCycle2Invoice), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(CommercialBillingCycle2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CommercialBillingCycle2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CommercialBillingCycle2.updated' }));
  });
