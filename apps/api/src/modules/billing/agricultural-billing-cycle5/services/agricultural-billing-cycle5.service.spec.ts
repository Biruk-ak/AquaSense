 ...
 * Unit tests: AgriculturalBillingCycle5
 * @copyright Biruk-ak
 */
...
import { AgriculturalBillingCycle5Service } from './agricultural-billing-cycle5.service';
...
describe('AgriculturalBillingCycle5Service', () => {
  let service: AgriculturalBillingCycle5Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new AgriculturalBillingCycle5Invoice(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        AgriculturalBillingCycle5Service,
        { provide: getRepositoryToken(AgriculturalBillingCycle5Invoice), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(AgriculturalBillingCycle5Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'AgriculturalBillingCycle5.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'AgriculturalBillingCycle5.updated' }));
  });
