 ...
 * Unit tests: IndustrialBillingCycle3
 * @copyright Biruk-ak
 */
...
import { IndustrialBillingCycle3Service } from './industrial-billing-cycle3.service';
...
describe('IndustrialBillingCycle3Service', () => {
  let service: IndustrialBillingCycle3Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new IndustrialBillingCycle3Invoice(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        IndustrialBillingCycle3Service,
        { provide: getRepositoryToken(IndustrialBillingCycle3Invoice), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(IndustrialBillingCycle3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'IndustrialBillingCycle3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'IndustrialBillingCycle3.updated' }));
  });
