 ...
 * Unit tests: AgriculturalBillingCycle1
 * @copyright Biruk-ak
 */
...
import { AgriculturalBillingCycle1Service } from './agricultural-billing-cycle1.service';
...
describe('AgriculturalBillingCycle1Service', () => {
  let service: AgriculturalBillingCycle1Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new AgriculturalBillingCycle1Invoice(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        AgriculturalBillingCycle1Service,
        { provide: getRepositoryToken(AgriculturalBillingCycle1Invoice), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(AgriculturalBillingCycle1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'AgriculturalBillingCycle1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'AgriculturalBillingCycle1.updated' }));
  });
