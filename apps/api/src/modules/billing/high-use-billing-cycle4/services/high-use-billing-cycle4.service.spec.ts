 ...
 * Unit tests: HighUseBillingCycle4
 * @copyright Biruk-ak
 */
...
import { HighUseBillingCycle4Service } from './high-use-billing-cycle4.service';
...
describe('HighUseBillingCycle4Service', () => {
  let service: HighUseBillingCycle4Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new HighUseBillingCycle4Invoice(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        HighUseBillingCycle4Service,
        { provide: getRepositoryToken(HighUseBillingCycle4Invoice), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(HighUseBillingCycle4Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'HighUseBillingCycle4.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'HighUseBillingCycle4.updated' }));
  });
