 ...
 * Unit tests: CommercialBillingCycle4
 * @copyright Biruk-ak
 */
...
import { CommercialBillingCycle4Service } from './commercial-billing-cycle4.service';
...
describe('CommercialBillingCycle4Service', () => {
  let service: CommercialBillingCycle4Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new CommercialBillingCycle4Invoice(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        CommercialBillingCycle4Service,
        { provide: getRepositoryToken(CommercialBillingCycle4Invoice), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(CommercialBillingCycle4Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CommercialBillingCycle4.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CommercialBillingCycle4.updated' }));
  });
