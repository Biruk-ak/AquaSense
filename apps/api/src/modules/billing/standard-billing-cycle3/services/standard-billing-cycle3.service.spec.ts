 ...
 * Unit tests: StandardBillingCycle3
 * @copyright Biruk-ak
 */
...
import { StandardBillingCycle3Service } from './standard-billing-cycle3.service';
...
describe('StandardBillingCycle3Service', () => {
  let service: StandardBillingCycle3Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new StandardBillingCycle3Invoice(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        StandardBillingCycle3Service,
        { provide: getRepositoryToken(StandardBillingCycle3Invoice), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(StandardBillingCycle3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'StandardBillingCycle3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'StandardBillingCycle3.updated' }));
  });
