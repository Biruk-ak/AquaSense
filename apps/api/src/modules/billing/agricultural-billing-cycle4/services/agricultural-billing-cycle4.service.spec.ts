 ...
 * Unit tests: AgriculturalBillingCycle4
 * @copyright Biruk-ak
 */
...
import { AgriculturalBillingCycle4Service } from './agricultural-billing-cycle4.service';
...
describe('AgriculturalBillingCycle4Service', () => {
  let service: AgriculturalBillingCycle4Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new AgriculturalBillingCycle4Invoice(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        AgriculturalBillingCycle4Service,
        { provide: getRepositoryToken(AgriculturalBillingCycle4Invoice), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(AgriculturalBillingCycle4Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'AgriculturalBillingCycle4.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'AgriculturalBillingCycle4.updated' }));
  });
