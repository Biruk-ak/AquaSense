 ...
 * Unit tests: CommercialBillingCycle3
 * @copyright Biruk-ak
 */
...
import { CommercialBillingCycle3Service } from './commercial-billing-cycle3.service';
...
describe('CommercialBillingCycle3Service', () => {
  let service: CommercialBillingCycle3Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new CommercialBillingCycle3Invoice(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        CommercialBillingCycle3Service,
        { provide: getRepositoryToken(CommercialBillingCycle3Invoice), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(CommercialBillingCycle3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CommercialBillingCycle3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CommercialBillingCycle3.updated' }));
  });
