 ...
 * Unit tests: CommercialBillingCycle1
 * @copyright Biruk-ak
 */
...
import { CommercialBillingCycle1Service } from './commercial-billing-cycle1.service';
...
describe('CommercialBillingCycle1Service', () => {
  let service: CommercialBillingCycle1Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new CommercialBillingCycle1Invoice(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        CommercialBillingCycle1Service,
        { provide: getRepositoryToken(CommercialBillingCycle1Invoice), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(CommercialBillingCycle1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CommercialBillingCycle1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CommercialBillingCycle1.updated' }));
  });
