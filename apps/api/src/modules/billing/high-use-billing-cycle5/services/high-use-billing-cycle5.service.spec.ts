 ...
 * Unit tests: HighUseBillingCycle5
 * @copyright Biruk-ak
 */
...
import { HighUseBillingCycle5Service } from './high-use-billing-cycle5.service';
...
describe('HighUseBillingCycle5Service', () => {
  let service: HighUseBillingCycle5Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new HighUseBillingCycle5Invoice(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        HighUseBillingCycle5Service,
        { provide: getRepositoryToken(HighUseBillingCycle5Invoice), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(HighUseBillingCycle5Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'HighUseBillingCycle5.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'HighUseBillingCycle5.updated' }));
  });
