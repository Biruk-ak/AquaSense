 ...
 * Unit tests: StandardBillingCycle1
 * @copyright Biruk-ak
 */
...
import { StandardBillingCycle1Service } from './standard-billing-cycle1.service';
...
describe('StandardBillingCycle1Service', () => {
  let service: StandardBillingCycle1Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new StandardBillingCycle1Invoice(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        StandardBillingCycle1Service,
        { provide: getRepositoryToken(StandardBillingCycle1Invoice), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(StandardBillingCycle1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'StandardBillingCycle1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'StandardBillingCycle1.updated' }));
  });
