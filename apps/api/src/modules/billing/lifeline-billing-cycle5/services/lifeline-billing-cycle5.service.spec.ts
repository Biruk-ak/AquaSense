 ...
 * Unit tests: LifelineBillingCycle5
 * @copyright Biruk-ak
 */
...
import { LifelineBillingCycle5Service } from './lifeline-billing-cycle5.service';
...
describe('LifelineBillingCycle5Service', () => {
  let service: LifelineBillingCycle5Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new LifelineBillingCycle5Invoice(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        LifelineBillingCycle5Service,
        { provide: getRepositoryToken(LifelineBillingCycle5Invoice), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(LifelineBillingCycle5Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LifelineBillingCycle5.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LifelineBillingCycle5.updated' }));
  });
