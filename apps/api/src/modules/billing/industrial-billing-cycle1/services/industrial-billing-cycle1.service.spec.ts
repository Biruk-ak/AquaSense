 ...
 * Unit tests: IndustrialBillingCycle1
 * @copyright Biruk-ak
 */
...
import { IndustrialBillingCycle1Service } from './industrial-billing-cycle1.service';
...
describe('IndustrialBillingCycle1Service', () => {
  let service: IndustrialBillingCycle1Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new IndustrialBillingCycle1Invoice(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        IndustrialBillingCycle1Service,
        { provide: getRepositoryToken(IndustrialBillingCycle1Invoice), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(IndustrialBillingCycle1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'IndustrialBillingCycle1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'IndustrialBillingCycle1.updated' }));
  });
