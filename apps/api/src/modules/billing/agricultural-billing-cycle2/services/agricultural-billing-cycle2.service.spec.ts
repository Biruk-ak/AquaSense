 ...
 * Unit tests: AgriculturalBillingCycle2
 * @copyright Biruk-ak
 */
...
import { AgriculturalBillingCycle2Service } from './agricultural-billing-cycle2.service';
...
describe('AgriculturalBillingCycle2Service', () => {
  let service: AgriculturalBillingCycle2Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new AgriculturalBillingCycle2Invoice(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        AgriculturalBillingCycle2Service,
        { provide: getRepositoryToken(AgriculturalBillingCycle2Invoice), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(AgriculturalBillingCycle2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'AgriculturalBillingCycle2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'AgriculturalBillingCycle2.updated' }));
  });
