 ...
 * Unit tests: StandardBillingCycle5
 * @copyright Biruk-ak
 */
...
import { StandardBillingCycle5Service } from './standard-billing-cycle5.service';
...
describe('StandardBillingCycle5Service', () => {
  let service: StandardBillingCycle5Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new StandardBillingCycle5Invoice(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        StandardBillingCycle5Service,
        { provide: getRepositoryToken(StandardBillingCycle5Invoice), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(StandardBillingCycle5Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'StandardBillingCycle5.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'StandardBillingCycle5.updated' }));
  });
