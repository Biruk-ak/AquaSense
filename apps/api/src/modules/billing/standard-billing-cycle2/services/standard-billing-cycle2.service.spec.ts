 ...
 * Unit tests: StandardBillingCycle2
 * @copyright Biruk-ak
 */
...
import { StandardBillingCycle2Service } from './standard-billing-cycle2.service';
...
describe('StandardBillingCycle2Service', () => {
  let service: StandardBillingCycle2Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new StandardBillingCycle2Invoice(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        StandardBillingCycle2Service,
        { provide: getRepositoryToken(StandardBillingCycle2Invoice), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(StandardBillingCycle2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'StandardBillingCycle2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'StandardBillingCycle2.updated' }));
  });
