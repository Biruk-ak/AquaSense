 ...
 * Unit tests: AgriculturalBillingCycle3
 * @copyright Biruk-ak
 */
...
import { AgriculturalBillingCycle3Service } from './agricultural-billing-cycle3.service';
...
describe('AgriculturalBillingCycle3Service', () => {
  let service: AgriculturalBillingCycle3Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new AgriculturalBillingCycle3Invoice(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        AgriculturalBillingCycle3Service,
        { provide: getRepositoryToken(AgriculturalBillingCycle3Invoice), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(AgriculturalBillingCycle3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'AgriculturalBillingCycle3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'AgriculturalBillingCycle3.updated' }));
  });
