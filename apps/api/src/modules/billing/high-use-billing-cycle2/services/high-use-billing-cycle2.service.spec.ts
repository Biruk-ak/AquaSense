 ...
 * Unit tests: HighUseBillingCycle2
 * @copyright Biruk-ak
 */
...
import { HighUseBillingCycle2Service } from './high-use-billing-cycle2.service';
...
describe('HighUseBillingCycle2Service', () => {
  let service: HighUseBillingCycle2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new HighUseBillingCycle2Invoice(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        HighUseBillingCycle2Service,
        { provide: getRepositoryToken(HighUseBillingCycle2Invoice), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(HighUseBillingCycle2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'HighUseBillingCycle2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'HighUseBillingCycle2.updated' }));
  });
