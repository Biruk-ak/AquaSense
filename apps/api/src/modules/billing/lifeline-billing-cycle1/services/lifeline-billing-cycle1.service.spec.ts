 ...
 * Unit tests: LifelineBillingCycle1
 * @copyright Biruk-ak
 */
...
import { LifelineBillingCycle1Service } from './lifeline-billing-cycle1.service';
...
describe('LifelineBillingCycle1Service', () => {
  let service: LifelineBillingCycle1Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new LifelineBillingCycle1Invoice(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        LifelineBillingCycle1Service,
        { provide: getRepositoryToken(LifelineBillingCycle1Invoice), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(LifelineBillingCycle1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LifelineBillingCycle1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LifelineBillingCycle1.updated' }));
  });
