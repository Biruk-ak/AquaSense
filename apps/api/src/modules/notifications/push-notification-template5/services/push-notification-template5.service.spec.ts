 ...
 * Unit tests: PushNotificationTemplate5
 * @copyright Biruk-ak
 */
...
import { PushNotificationTemplate5Service } from './push-notification-template5.service';
...
describe('PushNotificationTemplate5Service', () => {
  let service: PushNotificationTemplate5Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new PushNotificationTemplate5Message(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        PushNotificationTemplate5Service,
        { provide: getRepositoryToken(PushNotificationTemplate5Message), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(PushNotificationTemplate5Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PushNotificationTemplate5.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PushNotificationTemplate5.updated' }));
  });
