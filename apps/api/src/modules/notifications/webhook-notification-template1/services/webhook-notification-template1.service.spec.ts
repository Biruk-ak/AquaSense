 ...
 * Unit tests: WebhookNotificationTemplate1
 * @copyright Biruk-ak
 */
...
import { WebhookNotificationTemplate1Service } from './webhook-notification-template1.service';
...
describe('WebhookNotificationTemplate1Service', () => {
  let service: WebhookNotificationTemplate1Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new WebhookNotificationTemplate1Message(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        WebhookNotificationTemplate1Service,
        { provide: getRepositoryToken(WebhookNotificationTemplate1Message), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(WebhookNotificationTemplate1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'WebhookNotificationTemplate1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'WebhookNotificationTemplate1.updated' }));
  });
