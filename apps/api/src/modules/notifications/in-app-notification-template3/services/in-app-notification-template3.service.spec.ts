 ...
 * Unit tests: InAppNotificationTemplate3
 * @copyright Biruk-ak
 */
...
import { InAppNotificationTemplate3Service } from './in-app-notification-template3.service';
...
describe('InAppNotificationTemplate3Service', () => {
  let service: InAppNotificationTemplate3Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new InAppNotificationTemplate3Message(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        InAppNotificationTemplate3Service,
        { provide: getRepositoryToken(InAppNotificationTemplate3Message), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(InAppNotificationTemplate3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'InAppNotificationTemplate3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'InAppNotificationTemplate3.updated' }));
  });
