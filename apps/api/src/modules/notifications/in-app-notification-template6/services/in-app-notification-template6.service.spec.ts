 ...
 * Unit tests: InAppNotificationTemplate6
 * @copyright Biruk-ak
 */
...
import { InAppNotificationTemplate6Service } from './in-app-notification-template6.service';
...
describe('InAppNotificationTemplate6Service', () => {
  let service: InAppNotificationTemplate6Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new InAppNotificationTemplate6Message(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        InAppNotificationTemplate6Service,
        { provide: getRepositoryToken(InAppNotificationTemplate6Message), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(InAppNotificationTemplate6Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'InAppNotificationTemplate6.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'InAppNotificationTemplate6.updated' }));
  });
