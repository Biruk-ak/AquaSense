 ...
 * Unit tests: PushNotificationTemplate2
 * @copyright Biruk-ak
 */
...
import { PushNotificationTemplate2Service } from './push-notification-template2.service';
...
describe('PushNotificationTemplate2Service', () => {
  let service: PushNotificationTemplate2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new PushNotificationTemplate2Message(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        PushNotificationTemplate2Service,
        { provide: getRepositoryToken(PushNotificationTemplate2Message), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(PushNotificationTemplate2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PushNotificationTemplate2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PushNotificationTemplate2.updated' }));
  });
