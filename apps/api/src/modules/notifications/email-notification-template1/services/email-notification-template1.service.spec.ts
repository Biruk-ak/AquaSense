 ...
 * Unit tests: EmailNotificationTemplate1
 * @copyright Biruk-ak
 */
...
import { EmailNotificationTemplate1Service } from './email-notification-template1.service';
...
describe('EmailNotificationTemplate1Service', () => {
  let service: EmailNotificationTemplate1Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new EmailNotificationTemplate1Message(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        EmailNotificationTemplate1Service,
        { provide: getRepositoryToken(EmailNotificationTemplate1Message), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(EmailNotificationTemplate1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'EmailNotificationTemplate1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'EmailNotificationTemplate1.updated' }));
  });
