 ...
 * Unit tests: SmsNotificationTemplate4
 * @copyright Biruk-ak
 */
...
import { SmsNotificationTemplate4Service } from './sms-notification-template4.service';
...
describe('SmsNotificationTemplate4Service', () => {
  let service: SmsNotificationTemplate4Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new SmsNotificationTemplate4Message(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        SmsNotificationTemplate4Service,
        { provide: getRepositoryToken(SmsNotificationTemplate4Message), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(SmsNotificationTemplate4Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'SmsNotificationTemplate4.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'SmsNotificationTemplate4.updated' }));
  });
