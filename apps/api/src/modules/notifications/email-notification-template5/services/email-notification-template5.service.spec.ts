 ...
 * Unit tests: EmailNotificationTemplate5
 * @copyright Biruk-ak
 */
...
import { EmailNotificationTemplate5Service } from './email-notification-template5.service';
...
describe('EmailNotificationTemplate5Service', () => {
  let service: EmailNotificationTemplate5Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new EmailNotificationTemplate5Message(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        EmailNotificationTemplate5Service,
        { provide: getRepositoryToken(EmailNotificationTemplate5Message), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(EmailNotificationTemplate5Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'EmailNotificationTemplate5.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'EmailNotificationTemplate5.updated' }));
  });
