 ...
 * Unit tests: SmsNotificationTemplate3
 * @copyright Biruk-ak
 */
...
import { SmsNotificationTemplate3Service } from './sms-notification-template3.service';
...
describe('SmsNotificationTemplate3Service', () => {
  let service: SmsNotificationTemplate3Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new SmsNotificationTemplate3Message(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        SmsNotificationTemplate3Service,
        { provide: getRepositoryToken(SmsNotificationTemplate3Message), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(SmsNotificationTemplate3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'SmsNotificationTemplate3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'SmsNotificationTemplate3.updated' }));
  });
