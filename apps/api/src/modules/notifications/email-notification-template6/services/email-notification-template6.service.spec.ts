 ...
 * Unit tests: EmailNotificationTemplate6
 * @copyright Biruk-ak
 */
...
import { EmailNotificationTemplate6Service } from './email-notification-template6.service';
...
describe('EmailNotificationTemplate6Service', () => {
  let service: EmailNotificationTemplate6Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new EmailNotificationTemplate6Message(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        EmailNotificationTemplate6Service,
        { provide: getRepositoryToken(EmailNotificationTemplate6Message), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(EmailNotificationTemplate6Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'EmailNotificationTemplate6.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'EmailNotificationTemplate6.updated' }));
  });
