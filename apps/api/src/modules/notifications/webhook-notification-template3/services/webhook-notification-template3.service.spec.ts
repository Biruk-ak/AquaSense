 ...
 * Unit tests: WebhookNotificationTemplate3
 * @copyright Biruk-ak
 */
...
import { WebhookNotificationTemplate3Service } from './webhook-notification-template3.service';
...
describe('WebhookNotificationTemplate3Service', () => {
  let service: WebhookNotificationTemplate3Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new WebhookNotificationTemplate3Message(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        WebhookNotificationTemplate3Service,
        { provide: getRepositoryToken(WebhookNotificationTemplate3Message), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(WebhookNotificationTemplate3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'WebhookNotificationTemplate3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'WebhookNotificationTemplate3.updated' }));
  });
