 ...
 * Unit tests: EmailNotificationTemplate3
 * @copyright Biruk-ak
 */
...
import { EmailNotificationTemplate3Service } from './email-notification-template3.service';
...
describe('EmailNotificationTemplate3Service', () => {
  let service: EmailNotificationTemplate3Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new EmailNotificationTemplate3Message(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        EmailNotificationTemplate3Service,
        { provide: getRepositoryToken(EmailNotificationTemplate3Message), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(EmailNotificationTemplate3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'EmailNotificationTemplate3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'EmailNotificationTemplate3.updated' }));
  });
