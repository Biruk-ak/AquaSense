 ...
 * Unit tests: SmsNotificationTemplate1
 * @copyright Biruk-ak
 */
...
import { SmsNotificationTemplate1Service } from './sms-notification-template1.service';
...
describe('SmsNotificationTemplate1Service', () => {
  let service: SmsNotificationTemplate1Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new SmsNotificationTemplate1Message(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        SmsNotificationTemplate1Service,
        { provide: getRepositoryToken(SmsNotificationTemplate1Message), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(SmsNotificationTemplate1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'SmsNotificationTemplate1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'SmsNotificationTemplate1.updated' }));
  });
