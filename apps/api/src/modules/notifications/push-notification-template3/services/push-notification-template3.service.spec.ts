 ...
 * Unit tests: PushNotificationTemplate3
 * @copyright Biruk-ak
 */
...
import { PushNotificationTemplate3Service } from './push-notification-template3.service';
...
describe('PushNotificationTemplate3Service', () => {
  let service: PushNotificationTemplate3Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new PushNotificationTemplate3Message(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        PushNotificationTemplate3Service,
        { provide: getRepositoryToken(PushNotificationTemplate3Message), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(PushNotificationTemplate3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PushNotificationTemplate3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PushNotificationTemplate3.updated' }));
  });
