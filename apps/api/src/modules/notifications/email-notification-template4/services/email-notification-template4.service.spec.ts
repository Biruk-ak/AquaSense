 ...
 * Unit tests: EmailNotificationTemplate4
 * @copyright Biruk-ak
 */
...
import { EmailNotificationTemplate4Service } from './email-notification-template4.service';
...
describe('EmailNotificationTemplate4Service', () => {
  let service: EmailNotificationTemplate4Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new EmailNotificationTemplate4Message(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        EmailNotificationTemplate4Service,
        { provide: getRepositoryToken(EmailNotificationTemplate4Message), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(EmailNotificationTemplate4Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'EmailNotificationTemplate4.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'EmailNotificationTemplate4.updated' }));
  });
