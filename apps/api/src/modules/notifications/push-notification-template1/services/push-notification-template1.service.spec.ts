 ...
 * Unit tests: PushNotificationTemplate1
 * @copyright Biruk-ak
 */
...
import { PushNotificationTemplate1Service } from './push-notification-template1.service';
...
describe('PushNotificationTemplate1Service', () => {
  let service: PushNotificationTemplate1Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new PushNotificationTemplate1Message(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        PushNotificationTemplate1Service,
        { provide: getRepositoryToken(PushNotificationTemplate1Message), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(PushNotificationTemplate1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PushNotificationTemplate1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PushNotificationTemplate1.updated' }));
  });
