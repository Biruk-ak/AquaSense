 ...
 * Unit tests: InAppNotificationTemplate2
 * @copyright Biruk-ak
 */
...
import { InAppNotificationTemplate2Service } from './in-app-notification-template2.service';
...
describe('InAppNotificationTemplate2Service', () => {
  let service: InAppNotificationTemplate2Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new InAppNotificationTemplate2Message(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        InAppNotificationTemplate2Service,
        { provide: getRepositoryToken(InAppNotificationTemplate2Message), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(InAppNotificationTemplate2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'InAppNotificationTemplate2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'InAppNotificationTemplate2.updated' }));
  });
