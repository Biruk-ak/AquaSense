 ...
 * Unit tests: WebhookNotificationTemplate4
 * @copyright Biruk-ak
 */
...
import { WebhookNotificationTemplate4Service } from './webhook-notification-template4.service';
...
describe('WebhookNotificationTemplate4Service', () => {
  let service: WebhookNotificationTemplate4Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new WebhookNotificationTemplate4Message(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        WebhookNotificationTemplate4Service,
        { provide: getRepositoryToken(WebhookNotificationTemplate4Message), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(WebhookNotificationTemplate4Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'WebhookNotificationTemplate4.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'WebhookNotificationTemplate4.updated' }));
  });
