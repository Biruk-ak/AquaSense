 ...
 * Unit tests: SmsNotificationTemplate2
 * @copyright Biruk-ak
 */
...
import { SmsNotificationTemplate2Service } from './sms-notification-template2.service';
...
describe('SmsNotificationTemplate2Service', () => {
  let service: SmsNotificationTemplate2Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new SmsNotificationTemplate2Message(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        SmsNotificationTemplate2Service,
        { provide: getRepositoryToken(SmsNotificationTemplate2Message), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(SmsNotificationTemplate2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'SmsNotificationTemplate2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'SmsNotificationTemplate2.updated' }));
  });
