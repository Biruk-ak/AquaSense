 ...
 * Unit tests: SmsNotificationTemplate6
 * @copyright Biruk-ak
 */
...
import { SmsNotificationTemplate6Service } from './sms-notification-template6.service';
...
describe('SmsNotificationTemplate6Service', () => {
  let service: SmsNotificationTemplate6Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new SmsNotificationTemplate6Message(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        SmsNotificationTemplate6Service,
        { provide: getRepositoryToken(SmsNotificationTemplate6Message), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(SmsNotificationTemplate6Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'SmsNotificationTemplate6.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'SmsNotificationTemplate6.updated' }));
  });
