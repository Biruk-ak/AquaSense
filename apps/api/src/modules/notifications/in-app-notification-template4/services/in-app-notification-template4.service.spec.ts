 ...
 * Unit tests: InAppNotificationTemplate4
 * @copyright Biruk-ak
 */
...
import { InAppNotificationTemplate4Service } from './in-app-notification-template4.service';
...
describe('InAppNotificationTemplate4Service', () => {
  let service: InAppNotificationTemplate4Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new InAppNotificationTemplate4Message(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        InAppNotificationTemplate4Service,
        { provide: getRepositoryToken(InAppNotificationTemplate4Message), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(InAppNotificationTemplate4Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'InAppNotificationTemplate4.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'InAppNotificationTemplate4.updated' }));
  });
