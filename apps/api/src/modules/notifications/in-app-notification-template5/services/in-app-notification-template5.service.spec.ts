 ...
 * Unit tests: InAppNotificationTemplate5
 * @copyright Biruk-ak
 */
...
import { InAppNotificationTemplate5Service } from './in-app-notification-template5.service';
...
describe('InAppNotificationTemplate5Service', () => {
  let service: InAppNotificationTemplate5Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new InAppNotificationTemplate5Message(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        InAppNotificationTemplate5Service,
        { provide: getRepositoryToken(InAppNotificationTemplate5Message), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(InAppNotificationTemplate5Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'InAppNotificationTemplate5.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'InAppNotificationTemplate5.updated' }));
  });
