 ...
 * Unit tests: WebhookNotificationTemplate6
 * @copyright Biruk-ak
 */
...
import { WebhookNotificationTemplate6Service } from './webhook-notification-template6.service';
...
describe('WebhookNotificationTemplate6Service', () => {
  let service: WebhookNotificationTemplate6Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new WebhookNotificationTemplate6Message(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        WebhookNotificationTemplate6Service,
        { provide: getRepositoryToken(WebhookNotificationTemplate6Message), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(WebhookNotificationTemplate6Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'WebhookNotificationTemplate6.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'WebhookNotificationTemplate6.updated' }));
  });
