 ...
 * Unit tests: PushNotificationTemplate6
 * @copyright Biruk-ak
 */
...
import { PushNotificationTemplate6Service } from './push-notification-template6.service';
...
describe('PushNotificationTemplate6Service', () => {
  let service: PushNotificationTemplate6Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new PushNotificationTemplate6Message(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        PushNotificationTemplate6Service,
        { provide: getRepositoryToken(PushNotificationTemplate6Message), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(PushNotificationTemplate6Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PushNotificationTemplate6.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PushNotificationTemplate6.updated' }));
  });
