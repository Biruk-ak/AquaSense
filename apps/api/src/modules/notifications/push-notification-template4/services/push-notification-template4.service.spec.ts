 ...
 * Unit tests: PushNotificationTemplate4
 * @copyright Biruk-ak
 */
...
import { PushNotificationTemplate4Service } from './push-notification-template4.service';
...
describe('PushNotificationTemplate4Service', () => {
  let service: PushNotificationTemplate4Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new PushNotificationTemplate4Message(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        PushNotificationTemplate4Service,
        { provide: getRepositoryToken(PushNotificationTemplate4Message), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(PushNotificationTemplate4Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PushNotificationTemplate4.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PushNotificationTemplate4.updated' }));
  });
