 ...
 * Unit tests: WebhookNotificationTemplate2
 * @copyright Biruk-ak
 */
...
import { WebhookNotificationTemplate2Service } from './webhook-notification-template2.service';
...
describe('WebhookNotificationTemplate2Service', () => {
  let service: WebhookNotificationTemplate2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new WebhookNotificationTemplate2Message(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        WebhookNotificationTemplate2Service,
        { provide: getRepositoryToken(WebhookNotificationTemplate2Message), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(WebhookNotificationTemplate2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'WebhookNotificationTemplate2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'WebhookNotificationTemplate2.updated' }));
  });
