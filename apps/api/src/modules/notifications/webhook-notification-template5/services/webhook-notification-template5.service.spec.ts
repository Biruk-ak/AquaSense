 ...
 * Unit tests: WebhookNotificationTemplate5
 * @copyright Biruk-ak
 */
...
import { WebhookNotificationTemplate5Service } from './webhook-notification-template5.service';
...
describe('WebhookNotificationTemplate5Service', () => {
  let service: WebhookNotificationTemplate5Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new WebhookNotificationTemplate5Message(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        WebhookNotificationTemplate5Service,
        { provide: getRepositoryToken(WebhookNotificationTemplate5Message), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(WebhookNotificationTemplate5Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'WebhookNotificationTemplate5.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'WebhookNotificationTemplate5.updated' }));
  });
