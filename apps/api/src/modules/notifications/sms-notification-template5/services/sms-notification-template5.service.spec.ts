 ...
 * Unit tests: SmsNotificationTemplate5
 * @copyright Biruk-ak
 */
...
import { SmsNotificationTemplate5Service } from './sms-notification-template5.service';
...
describe('SmsNotificationTemplate5Service', () => {
  let service: SmsNotificationTemplate5Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new SmsNotificationTemplate5Message(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        SmsNotificationTemplate5Service,
        { provide: getRepositoryToken(SmsNotificationTemplate5Message), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(SmsNotificationTemplate5Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'SmsNotificationTemplate5.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'SmsNotificationTemplate5.updated' }));
  });
