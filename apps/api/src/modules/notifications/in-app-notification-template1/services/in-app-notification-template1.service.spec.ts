 ...
 * Unit tests: InAppNotificationTemplate1
 * @copyright Biruk-ak
 */
...
import { InAppNotificationTemplate1Service } from './in-app-notification-template1.service';
...
describe('InAppNotificationTemplate1Service', () => {
  let service: InAppNotificationTemplate1Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new InAppNotificationTemplate1Message(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        InAppNotificationTemplate1Service,
        { provide: getRepositoryToken(InAppNotificationTemplate1Message), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(InAppNotificationTemplate1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'InAppNotificationTemplate1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'InAppNotificationTemplate1.updated' }));
  });
