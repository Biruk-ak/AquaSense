...
import { LeakZone07Service } from './leak-zone07.service';
import { LeakZone07Alert } from '../entities/leak-zone07-alert.entity';
import { AuditService, EventBusService } from '@aquasense/common';

describe('LeakZone07Service', () => {
  let service: LeakZone07Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new LeakZone07Alert(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        LeakZone07Service,
        { provide: getRepositoryToken(LeakZone07Alert), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(LeakZone07Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakZone07.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakZone07.updated' }));
  });
