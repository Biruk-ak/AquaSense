...
import { LeakZone19Service } from './leak-zone19.service';
import { LeakZone19Alert } from '../entities/leak-zone19-alert.entity';
import { AuditService, EventBusService } from '@aquasense/common';

describe('LeakZone19Service', () => {
  let service: LeakZone19Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new LeakZone19Alert(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        LeakZone19Service,
        { provide: getRepositoryToken(LeakZone19Alert), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(LeakZone19Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakZone19.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakZone19.updated' }));
  });
