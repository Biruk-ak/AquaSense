...
import { LeakZone01Service } from './leak-zone01.service';
import { LeakZone01Alert } from '../entities/leak-zone01-alert.entity';
import { AuditService, EventBusService } from '@aquasense/common';

describe('LeakZone01Service', () => {
  let service: LeakZone01Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new LeakZone01Alert(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        LeakZone01Service,
        { provide: getRepositoryToken(LeakZone01Alert), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(LeakZone01Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakZone01.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakZone01.updated' }));
  });
