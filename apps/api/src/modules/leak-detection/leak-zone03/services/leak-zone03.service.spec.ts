...
import { LeakZone03Service } from './leak-zone03.service';
import { LeakZone03Alert } from '../entities/leak-zone03-alert.entity';
import { AuditService, EventBusService } from '@aquasense/common';

describe('LeakZone03Service', () => {
  let service: LeakZone03Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new LeakZone03Alert(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        LeakZone03Service,
        { provide: getRepositoryToken(LeakZone03Alert), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(LeakZone03Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakZone03.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakZone03.updated' }));
  });
