...
import { LeakZone16Service } from './leak-zone16.service';
import { LeakZone16Alert } from '../entities/leak-zone16-alert.entity';
import { AuditService, EventBusService } from '@aquasense/common';

describe('LeakZone16Service', () => {
  let service: LeakZone16Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new LeakZone16Alert(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        LeakZone16Service,
        { provide: getRepositoryToken(LeakZone16Alert), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(LeakZone16Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakZone16.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakZone16.updated' }));
  });
