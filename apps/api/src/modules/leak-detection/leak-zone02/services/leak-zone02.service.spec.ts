...
import { LeakZone02Service } from './leak-zone02.service';
import { LeakZone02Alert } from '../entities/leak-zone02-alert.entity';
import { AuditService, EventBusService } from '@aquasense/common';

describe('LeakZone02Service', () => {
  let service: LeakZone02Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new LeakZone02Alert(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        LeakZone02Service,
        { provide: getRepositoryToken(LeakZone02Alert), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(LeakZone02Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakZone02.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakZone02.updated' }));
  });
