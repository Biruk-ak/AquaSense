...
import { LeakZone17Service } from './leak-zone17.service';
import { LeakZone17Alert } from '../entities/leak-zone17-alert.entity';
import { AuditService, EventBusService } from '@aquasense/common';

describe('LeakZone17Service', () => {
  let service: LeakZone17Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new LeakZone17Alert(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        LeakZone17Service,
        { provide: getRepositoryToken(LeakZone17Alert), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(LeakZone17Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakZone17.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakZone17.updated' }));
  });
