...
import { LeakZone21Service } from './leak-zone21.service';
import { LeakZone21Alert } from '../entities/leak-zone21-alert.entity';
import { AuditService, EventBusService } from '@aquasense/common';

describe('LeakZone21Service', () => {
  let service: LeakZone21Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new LeakZone21Alert(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        LeakZone21Service,
        { provide: getRepositoryToken(LeakZone21Alert), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(LeakZone21Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakZone21.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakZone21.updated' }));
  });
