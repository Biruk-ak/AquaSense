...
import { LeakZone14Service } from './leak-zone14.service';
import { LeakZone14Alert } from '../entities/leak-zone14-alert.entity';
import { AuditService, EventBusService } from '@aquasense/common';

describe('LeakZone14Service', () => {
  let service: LeakZone14Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new LeakZone14Alert(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        LeakZone14Service,
        { provide: getRepositoryToken(LeakZone14Alert), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(LeakZone14Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakZone14.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakZone14.updated' }));
  });
