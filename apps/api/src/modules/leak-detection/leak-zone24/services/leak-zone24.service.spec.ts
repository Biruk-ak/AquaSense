...
import { LeakZone24Service } from './leak-zone24.service';
import { LeakZone24Alert } from '../entities/leak-zone24-alert.entity';
import { AuditService, EventBusService } from '@aquasense/common';

describe('LeakZone24Service', () => {
  let service: LeakZone24Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new LeakZone24Alert(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        LeakZone24Service,
        { provide: getRepositoryToken(LeakZone24Alert), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(LeakZone24Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakZone24.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakZone24.updated' }));
  });
