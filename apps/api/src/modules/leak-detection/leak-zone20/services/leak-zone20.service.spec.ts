...
import { LeakZone20Service } from './leak-zone20.service';
import { LeakZone20Alert } from '../entities/leak-zone20-alert.entity';
import { AuditService, EventBusService } from '@aquasense/common';

describe('LeakZone20Service', () => {
  let service: LeakZone20Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new LeakZone20Alert(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        LeakZone20Service,
        { provide: getRepositoryToken(LeakZone20Alert), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(LeakZone20Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakZone20.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakZone20.updated' }));
  });
