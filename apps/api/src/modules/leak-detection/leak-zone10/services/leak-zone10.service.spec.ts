...
import { LeakZone10Service } from './leak-zone10.service';
import { LeakZone10Alert } from '../entities/leak-zone10-alert.entity';
import { AuditService, EventBusService } from '@aquasense/common';

describe('LeakZone10Service', () => {
  let service: LeakZone10Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new LeakZone10Alert(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        LeakZone10Service,
        { provide: getRepositoryToken(LeakZone10Alert), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(LeakZone10Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakZone10.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakZone10.updated' }));
  });
