...
import { LeakZone18Service } from './leak-zone18.service';
import { LeakZone18Alert } from '../entities/leak-zone18-alert.entity';
import { AuditService, EventBusService } from '@aquasense/common';

describe('LeakZone18Service', () => {
  let service: LeakZone18Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new LeakZone18Alert(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        LeakZone18Service,
        { provide: getRepositoryToken(LeakZone18Alert), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(LeakZone18Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakZone18.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakZone18.updated' }));
  });
