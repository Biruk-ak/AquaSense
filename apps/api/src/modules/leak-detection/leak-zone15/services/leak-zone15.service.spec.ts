...
import { LeakZone15Service } from './leak-zone15.service';
import { LeakZone15Alert } from '../entities/leak-zone15-alert.entity';
import { AuditService, EventBusService } from '@aquasense/common';

describe('LeakZone15Service', () => {
  let service: LeakZone15Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new LeakZone15Alert(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        LeakZone15Service,
        { provide: getRepositoryToken(LeakZone15Alert), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(LeakZone15Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakZone15.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakZone15.updated' }));
  });
