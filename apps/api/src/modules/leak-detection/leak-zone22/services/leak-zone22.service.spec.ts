...
import { LeakZone22Service } from './leak-zone22.service';
import { LeakZone22Alert } from '../entities/leak-zone22-alert.entity';
import { AuditService, EventBusService } from '@aquasense/common';

describe('LeakZone22Service', () => {
  let service: LeakZone22Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new LeakZone22Alert(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        LeakZone22Service,
        { provide: getRepositoryToken(LeakZone22Alert), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(LeakZone22Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakZone22.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakZone22.updated' }));
  });
