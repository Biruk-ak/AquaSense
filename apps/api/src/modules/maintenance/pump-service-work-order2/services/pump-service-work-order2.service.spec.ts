 ...
 * Unit tests: PumpServiceWorkOrder2
 * @copyright Biruk-ak
 */
...
import { PumpServiceWorkOrder2Service } from './pump-service-work-order2.service';
...
describe('PumpServiceWorkOrder2Service', () => {
  let service: PumpServiceWorkOrder2Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new PumpServiceWorkOrder2Ticket(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        PumpServiceWorkOrder2Service,
        { provide: getRepositoryToken(PumpServiceWorkOrder2Ticket), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(PumpServiceWorkOrder2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PumpServiceWorkOrder2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PumpServiceWorkOrder2.updated' }));
  });
