 ...
 * Unit tests: CorrectiveWorkOrder3
 * @copyright Biruk-ak
 */
...
import { CorrectiveWorkOrder3Service } from './corrective-work-order3.service';
...
describe('CorrectiveWorkOrder3Service', () => {
  let service: CorrectiveWorkOrder3Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new CorrectiveWorkOrder3Ticket(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        CorrectiveWorkOrder3Service,
        { provide: getRepositoryToken(CorrectiveWorkOrder3Ticket), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(CorrectiveWorkOrder3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CorrectiveWorkOrder3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CorrectiveWorkOrder3.updated' }));
  });
