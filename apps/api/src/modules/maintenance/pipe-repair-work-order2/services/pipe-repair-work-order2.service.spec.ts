 ...
 * Unit tests: PipeRepairWorkOrder2
 * @copyright Biruk-ak
 */
...
import { PipeRepairWorkOrder2Service } from './pipe-repair-work-order2.service';
...
describe('PipeRepairWorkOrder2Service', () => {
  let service: PipeRepairWorkOrder2Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new PipeRepairWorkOrder2Ticket(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        PipeRepairWorkOrder2Service,
        { provide: getRepositoryToken(PipeRepairWorkOrder2Ticket), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(PipeRepairWorkOrder2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PipeRepairWorkOrder2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PipeRepairWorkOrder2.updated' }));
  });
