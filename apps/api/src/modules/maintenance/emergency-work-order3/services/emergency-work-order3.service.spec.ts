 ...
 * Unit tests: EmergencyWorkOrder3
 * @copyright Biruk-ak
 */
...
import { EmergencyWorkOrder3Service } from './emergency-work-order3.service';
...
describe('EmergencyWorkOrder3Service', () => {
  let service: EmergencyWorkOrder3Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new EmergencyWorkOrder3Ticket(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        EmergencyWorkOrder3Service,
        { provide: getRepositoryToken(EmergencyWorkOrder3Ticket), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(EmergencyWorkOrder3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'EmergencyWorkOrder3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'EmergencyWorkOrder3.updated' }));
  });
