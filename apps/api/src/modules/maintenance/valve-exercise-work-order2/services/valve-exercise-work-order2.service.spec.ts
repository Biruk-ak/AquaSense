 ...
 * Unit tests: ValveExerciseWorkOrder2
 * @copyright Biruk-ak
 */
...
import { ValveExerciseWorkOrder2Service } from './valve-exercise-work-order2.service';
...
describe('ValveExerciseWorkOrder2Service', () => {
  let service: ValveExerciseWorkOrder2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new ValveExerciseWorkOrder2Ticket(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ValveExerciseWorkOrder2Service,
        { provide: getRepositoryToken(ValveExerciseWorkOrder2Ticket), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(ValveExerciseWorkOrder2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ValveExerciseWorkOrder2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ValveExerciseWorkOrder2.updated' }));
  });
