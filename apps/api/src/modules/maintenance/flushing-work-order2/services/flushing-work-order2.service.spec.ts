...
import { FlushingWorkOrder2Service } from './flushing-work-order2.service';
...
describe('FlushingWorkOrder2Service', () => {
  let service: FlushingWorkOrder2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new FlushingWorkOrder2Ticket(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        FlushingWorkOrder2Service,
        { provide: getRepositoryToken(FlushingWorkOrder2Ticket), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(FlushingWorkOrder2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'FlushingWorkOrder2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'FlushingWorkOrder2.updated' }));
  });
