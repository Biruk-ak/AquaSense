 ...
 * Unit tests: CalibrationWorkOrder3
 * @copyright Biruk-ak
 */
...
import { CalibrationWorkOrder3Service } from './calibration-work-order3.service';
...
describe('CalibrationWorkOrder3Service', () => {
  let service: CalibrationWorkOrder3Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new CalibrationWorkOrder3Ticket(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        CalibrationWorkOrder3Service,
        { provide: getRepositoryToken(CalibrationWorkOrder3Ticket), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(CalibrationWorkOrder3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CalibrationWorkOrder3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CalibrationWorkOrder3.updated' }));
  });
