 ...
 * Unit tests: EmergencyWorkOrder1
 * @copyright Biruk-ak
 */
...
import { EmergencyWorkOrder1Service } from './emergency-work-order1.service';
...
describe('EmergencyWorkOrder1Service', () => {
  let service: EmergencyWorkOrder1Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new EmergencyWorkOrder1Ticket(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        EmergencyWorkOrder1Service,
        { provide: getRepositoryToken(EmergencyWorkOrder1Ticket), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(EmergencyWorkOrder1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'EmergencyWorkOrder1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'EmergencyWorkOrder1.updated' }));
  });
