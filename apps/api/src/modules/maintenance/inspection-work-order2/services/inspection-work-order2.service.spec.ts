 ...
 * Unit tests: InspectionWorkOrder2
 * @copyright Biruk-ak
 */
...
import { InspectionWorkOrder2Service } from './inspection-work-order2.service';
...
describe('InspectionWorkOrder2Service', () => {
  let service: InspectionWorkOrder2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new InspectionWorkOrder2Ticket(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        InspectionWorkOrder2Service,
        { provide: getRepositoryToken(InspectionWorkOrder2Ticket), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(InspectionWorkOrder2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'InspectionWorkOrder2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'InspectionWorkOrder2.updated' }));
  });
