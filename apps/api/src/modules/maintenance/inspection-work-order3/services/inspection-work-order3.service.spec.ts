 ...
 * Unit tests: InspectionWorkOrder3
 * @copyright Biruk-ak
 */
...
import { InspectionWorkOrder3Service } from './inspection-work-order3.service';
...
describe('InspectionWorkOrder3Service', () => {
  let service: InspectionWorkOrder3Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new InspectionWorkOrder3Ticket(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        InspectionWorkOrder3Service,
        { provide: getRepositoryToken(InspectionWorkOrder3Ticket), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(InspectionWorkOrder3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'InspectionWorkOrder3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'InspectionWorkOrder3.updated' }));
  });
