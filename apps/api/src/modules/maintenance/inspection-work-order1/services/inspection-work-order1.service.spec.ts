 ...
 * Unit tests: InspectionWorkOrder1
 * @copyright Biruk-ak
 */
...
import { InspectionWorkOrder1Service } from './inspection-work-order1.service';
...
describe('InspectionWorkOrder1Service', () => {
  let service: InspectionWorkOrder1Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new InspectionWorkOrder1Ticket(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        InspectionWorkOrder1Service,
        { provide: getRepositoryToken(InspectionWorkOrder1Ticket), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(InspectionWorkOrder1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'InspectionWorkOrder1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'InspectionWorkOrder1.updated' }));
  });
