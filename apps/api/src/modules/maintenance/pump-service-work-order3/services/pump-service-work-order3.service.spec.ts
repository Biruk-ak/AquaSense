 ...
 * Unit tests: PumpServiceWorkOrder3
 * @copyright Biruk-ak
 */
...
import { PumpServiceWorkOrder3Service } from './pump-service-work-order3.service';
...
describe('PumpServiceWorkOrder3Service', () => {
  let service: PumpServiceWorkOrder3Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new PumpServiceWorkOrder3Ticket(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        PumpServiceWorkOrder3Service,
        { provide: getRepositoryToken(PumpServiceWorkOrder3Ticket), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(PumpServiceWorkOrder3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PumpServiceWorkOrder3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PumpServiceWorkOrder3.updated' }));
  });
