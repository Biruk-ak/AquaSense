 ...
 * Unit tests: CorrectiveWorkOrder2
 * @copyright Biruk-ak
 */
...
import { CorrectiveWorkOrder2Service } from './corrective-work-order2.service';
...
describe('CorrectiveWorkOrder2Service', () => {
  let service: CorrectiveWorkOrder2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new CorrectiveWorkOrder2Ticket(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        CorrectiveWorkOrder2Service,
        { provide: getRepositoryToken(CorrectiveWorkOrder2Ticket), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(CorrectiveWorkOrder2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CorrectiveWorkOrder2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CorrectiveWorkOrder2.updated' }));
  });
