 ...
 * Unit tests: PreventiveWorkOrder1
 * @copyright Biruk-ak
 */
...
import { PreventiveWorkOrder1Service } from './preventive-work-order1.service';
...
describe('PreventiveWorkOrder1Service', () => {
  let service: PreventiveWorkOrder1Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new PreventiveWorkOrder1Ticket(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        PreventiveWorkOrder1Service,
        { provide: getRepositoryToken(PreventiveWorkOrder1Ticket), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(PreventiveWorkOrder1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PreventiveWorkOrder1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PreventiveWorkOrder1.updated' }));
  });
