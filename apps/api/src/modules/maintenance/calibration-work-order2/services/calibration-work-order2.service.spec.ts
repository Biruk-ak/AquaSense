 ...
 * Unit tests: CalibrationWorkOrder2
 * @copyright Biruk-ak
 */
...
import { CalibrationWorkOrder2Service } from './calibration-work-order2.service';
...
describe('CalibrationWorkOrder2Service', () => {
  let service: CalibrationWorkOrder2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new CalibrationWorkOrder2Ticket(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        CalibrationWorkOrder2Service,
        { provide: getRepositoryToken(CalibrationWorkOrder2Ticket), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(CalibrationWorkOrder2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CalibrationWorkOrder2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CalibrationWorkOrder2.updated' }));
  });
