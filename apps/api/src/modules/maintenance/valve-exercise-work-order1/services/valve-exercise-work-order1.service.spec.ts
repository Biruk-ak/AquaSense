 ...
 * Unit tests: ValveExerciseWorkOrder1
 * @copyright Biruk-ak
 */
...
import { ValveExerciseWorkOrder1Service } from './valve-exercise-work-order1.service';
...
describe('ValveExerciseWorkOrder1Service', () => {
  let service: ValveExerciseWorkOrder1Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new ValveExerciseWorkOrder1Ticket(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ValveExerciseWorkOrder1Service,
        { provide: getRepositoryToken(ValveExerciseWorkOrder1Ticket), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(ValveExerciseWorkOrder1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ValveExerciseWorkOrder1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ValveExerciseWorkOrder1.updated' }));
  });
