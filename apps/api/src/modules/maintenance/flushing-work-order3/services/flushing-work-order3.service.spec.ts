...
import { FlushingWorkOrder3Service } from './flushing-work-order3.service';
...
describe('FlushingWorkOrder3Service', () => {
  let service: FlushingWorkOrder3Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new FlushingWorkOrder3Ticket(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        FlushingWorkOrder3Service,
        { provide: getRepositoryToken(FlushingWorkOrder3Ticket), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(FlushingWorkOrder3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'FlushingWorkOrder3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'FlushingWorkOrder3.updated' }));
  });
