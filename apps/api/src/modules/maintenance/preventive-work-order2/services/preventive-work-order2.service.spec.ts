 ...
 * Unit tests: PreventiveWorkOrder2
 * @copyright Biruk-ak
 */
...
import { PreventiveWorkOrder2Service } from './preventive-work-order2.service';
...
describe('PreventiveWorkOrder2Service', () => {
  let service: PreventiveWorkOrder2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new PreventiveWorkOrder2Ticket(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        PreventiveWorkOrder2Service,
        { provide: getRepositoryToken(PreventiveWorkOrder2Ticket), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(PreventiveWorkOrder2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PreventiveWorkOrder2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PreventiveWorkOrder2.updated' }));
  });
