 ...
 * Unit tests: PipeRepairWorkOrder1
 * @copyright Biruk-ak
 */
...
import { PipeRepairWorkOrder1Service } from './pipe-repair-work-order1.service';
...
describe('PipeRepairWorkOrder1Service', () => {
  let service: PipeRepairWorkOrder1Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new PipeRepairWorkOrder1Ticket(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        PipeRepairWorkOrder1Service,
        { provide: getRepositoryToken(PipeRepairWorkOrder1Ticket), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(PipeRepairWorkOrder1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PipeRepairWorkOrder1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PipeRepairWorkOrder1.updated' }));
  });
