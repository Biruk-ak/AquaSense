 ...
 * Unit tests: PreventiveWorkOrder3
 * @copyright Biruk-ak
 */
...
import { PreventiveWorkOrder3Service } from './preventive-work-order3.service';
...
describe('PreventiveWorkOrder3Service', () => {
  let service: PreventiveWorkOrder3Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new PreventiveWorkOrder3Ticket(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        PreventiveWorkOrder3Service,
        { provide: getRepositoryToken(PreventiveWorkOrder3Ticket), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(PreventiveWorkOrder3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PreventiveWorkOrder3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PreventiveWorkOrder3.updated' }));
  });
