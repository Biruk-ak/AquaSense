 ...
 * Unit tests: HydrantTestWorkOrder2
 * @copyright Biruk-ak
 */
...
import { HydrantTestWorkOrder2Service } from './hydrant-test-work-order2.service';
...
describe('HydrantTestWorkOrder2Service', () => {
  let service: HydrantTestWorkOrder2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new HydrantTestWorkOrder2Ticket(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        HydrantTestWorkOrder2Service,
        { provide: getRepositoryToken(HydrantTestWorkOrder2Ticket), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(HydrantTestWorkOrder2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'HydrantTestWorkOrder2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'HydrantTestWorkOrder2.updated' }));
  });
