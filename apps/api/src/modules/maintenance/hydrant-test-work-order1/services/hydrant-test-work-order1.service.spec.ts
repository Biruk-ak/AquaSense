 ...
 * Unit tests: HydrantTestWorkOrder1
 * @copyright Biruk-ak
 */
...
import { HydrantTestWorkOrder1Service } from './hydrant-test-work-order1.service';
...
describe('HydrantTestWorkOrder1Service', () => {
  let service: HydrantTestWorkOrder1Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new HydrantTestWorkOrder1Ticket(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        HydrantTestWorkOrder1Service,
        { provide: getRepositoryToken(HydrantTestWorkOrder1Ticket), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(HydrantTestWorkOrder1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'HydrantTestWorkOrder1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'HydrantTestWorkOrder1.updated' }));
  });
