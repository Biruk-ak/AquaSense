 ...
 * Unit tests: PumpServiceWorkOrder1
 * @copyright Biruk-ak
 */
...
import { PumpServiceWorkOrder1Service } from './pump-service-work-order1.service';
...
describe('PumpServiceWorkOrder1Service', () => {
  let service: PumpServiceWorkOrder1Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new PumpServiceWorkOrder1Ticket(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        PumpServiceWorkOrder1Service,
        { provide: getRepositoryToken(PumpServiceWorkOrder1Ticket), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(PumpServiceWorkOrder1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PumpServiceWorkOrder1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PumpServiceWorkOrder1.updated' }));
  });
