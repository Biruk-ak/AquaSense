 ...
 * Unit tests: CalibrationWorkOrder1
 * @copyright Biruk-ak
 */
...
import { CalibrationWorkOrder1Service } from './calibration-work-order1.service';
...
describe('CalibrationWorkOrder1Service', () => {
  let service: CalibrationWorkOrder1Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new CalibrationWorkOrder1Ticket(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        CalibrationWorkOrder1Service,
        { provide: getRepositoryToken(CalibrationWorkOrder1Ticket), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(CalibrationWorkOrder1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CalibrationWorkOrder1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CalibrationWorkOrder1.updated' }));
  });
