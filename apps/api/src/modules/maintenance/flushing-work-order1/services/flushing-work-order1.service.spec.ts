...
import { FlushingWorkOrder1Service } from './flushing-work-order1.service';
...
describe('FlushingWorkOrder1Service', () => {
  let service: FlushingWorkOrder1Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new FlushingWorkOrder1Ticket(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        FlushingWorkOrder1Service,
        { provide: getRepositoryToken(FlushingWorkOrder1Ticket), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(FlushingWorkOrder1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'FlushingWorkOrder1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'FlushingWorkOrder1.updated' }));
  });
