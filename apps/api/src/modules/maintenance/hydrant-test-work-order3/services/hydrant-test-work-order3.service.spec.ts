 ...
 * Unit tests: HydrantTestWorkOrder3
 * @copyright Biruk-ak
 */
...
import { HydrantTestWorkOrder3Service } from './hydrant-test-work-order3.service';
...
describe('HydrantTestWorkOrder3Service', () => {
  let service: HydrantTestWorkOrder3Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new HydrantTestWorkOrder3Ticket(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        HydrantTestWorkOrder3Service,
        { provide: getRepositoryToken(HydrantTestWorkOrder3Ticket), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(HydrantTestWorkOrder3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'HydrantTestWorkOrder3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'HydrantTestWorkOrder3.updated' }));
  });
