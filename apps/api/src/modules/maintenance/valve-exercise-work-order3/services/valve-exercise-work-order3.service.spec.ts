 ...
 * Unit tests: ValveExerciseWorkOrder3
 * @copyright Biruk-ak
 */
...
import { ValveExerciseWorkOrder3Service } from './valve-exercise-work-order3.service';
...
describe('ValveExerciseWorkOrder3Service', () => {
  let service: ValveExerciseWorkOrder3Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new ValveExerciseWorkOrder3Ticket(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ValveExerciseWorkOrder3Service,
        { provide: getRepositoryToken(ValveExerciseWorkOrder3Ticket), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(ValveExerciseWorkOrder3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ValveExerciseWorkOrder3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ValveExerciseWorkOrder3.updated' }));
  });
