 ...
 * Unit tests: CorrectiveWorkOrder1
 * @copyright Biruk-ak
 */
...
import { CorrectiveWorkOrder1Service } from './corrective-work-order1.service';
...
describe('CorrectiveWorkOrder1Service', () => {
  let service: CorrectiveWorkOrder1Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new CorrectiveWorkOrder1Ticket(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        CorrectiveWorkOrder1Service,
        { provide: getRepositoryToken(CorrectiveWorkOrder1Ticket), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(CorrectiveWorkOrder1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CorrectiveWorkOrder1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CorrectiveWorkOrder1.updated' }));
  });
