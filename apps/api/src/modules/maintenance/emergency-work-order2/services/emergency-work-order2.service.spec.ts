 ...
 * Unit tests: EmergencyWorkOrder2
 * @copyright Biruk-ak
 */
...
import { EmergencyWorkOrder2Service } from './emergency-work-order2.service';
...
describe('EmergencyWorkOrder2Service', () => {
  let service: EmergencyWorkOrder2Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new EmergencyWorkOrder2Ticket(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        EmergencyWorkOrder2Service,
        { provide: getRepositoryToken(EmergencyWorkOrder2Ticket), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(EmergencyWorkOrder2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'EmergencyWorkOrder2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'EmergencyWorkOrder2.updated' }));
  });
