 ...
 * Unit tests: PipeRepairWorkOrder3
 * @copyright Biruk-ak
 */
...
import { PipeRepairWorkOrder3Service } from './pipe-repair-work-order3.service';
...
describe('PipeRepairWorkOrder3Service', () => {
  let service: PipeRepairWorkOrder3Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new PipeRepairWorkOrder3Ticket(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        PipeRepairWorkOrder3Service,
        { provide: getRepositoryToken(PipeRepairWorkOrder3Ticket), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(PipeRepairWorkOrder3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PipeRepairWorkOrder3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PipeRepairWorkOrder3.updated' }));
  });
