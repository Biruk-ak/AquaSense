 ...
 * Unit tests: QualityAnalyticsSlice4
 * @copyright Biruk-ak
 */
...
import { QualityAnalyticsSlice4Service } from './quality-analytics-slice4.service';
...
describe('QualityAnalyticsSlice4Service', () => {
  let service: QualityAnalyticsSlice4Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new QualityAnalyticsSlice4Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        QualityAnalyticsSlice4Service,
        { provide: getRepositoryToken(QualityAnalyticsSlice4Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(QualityAnalyticsSlice4Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'QualityAnalyticsSlice4.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'QualityAnalyticsSlice4.updated' }));
  });
