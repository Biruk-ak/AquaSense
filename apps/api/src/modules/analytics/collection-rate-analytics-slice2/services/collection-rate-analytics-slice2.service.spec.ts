 ...
 * Unit tests: CollectionRateAnalyticsSlice2
 * @copyright Biruk-ak
 */
...
import { CollectionRateAnalyticsSlice2Service } from './collection-rate-analytics-slice2.service';
...
describe('CollectionRateAnalyticsSlice2Service', () => {
  let service: CollectionRateAnalyticsSlice2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new CollectionRateAnalyticsSlice2Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        CollectionRateAnalyticsSlice2Service,
        { provide: getRepositoryToken(CollectionRateAnalyticsSlice2Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(CollectionRateAnalyticsSlice2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CollectionRateAnalyticsSlice2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CollectionRateAnalyticsSlice2.updated' }));
  });
