...
import { NrwAnalyticsSlice2Service } from './nrw-analytics-slice2.service';
...
describe('NrwAnalyticsSlice2Service', () => {
  let service: NrwAnalyticsSlice2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new NrwAnalyticsSlice2Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        NrwAnalyticsSlice2Service,
        { provide: getRepositoryToken(NrwAnalyticsSlice2Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(NrwAnalyticsSlice2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'NrwAnalyticsSlice2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'NrwAnalyticsSlice2.updated' }));
  });
