 ...
 * Unit tests: ConsumptionAnalyticsSlice2
 * @copyright Biruk-ak
 */
...
import { ConsumptionAnalyticsSlice2Service } from './consumption-analytics-slice2.service';
...
describe('ConsumptionAnalyticsSlice2Service', () => {
  let service: ConsumptionAnalyticsSlice2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new ConsumptionAnalyticsSlice2Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ConsumptionAnalyticsSlice2Service,
        { provide: getRepositoryToken(ConsumptionAnalyticsSlice2Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(ConsumptionAnalyticsSlice2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ConsumptionAnalyticsSlice2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ConsumptionAnalyticsSlice2.updated' }));
  });
