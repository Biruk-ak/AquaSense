 ...
 * Unit tests: RevenueAnalyticsSlice1
 * @copyright Biruk-ak
 */
...
import { RevenueAnalyticsSlice1Service } from './revenue-analytics-slice1.service';
...
describe('RevenueAnalyticsSlice1Service', () => {
  let service: RevenueAnalyticsSlice1Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new RevenueAnalyticsSlice1Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        RevenueAnalyticsSlice1Service,
        { provide: getRepositoryToken(RevenueAnalyticsSlice1Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(RevenueAnalyticsSlice1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'RevenueAnalyticsSlice1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'RevenueAnalyticsSlice1.updated' }));
  });
