 ...
 * Unit tests: CollectionRateAnalyticsSlice3
 * @copyright Biruk-ak
 */
...
import { CollectionRateAnalyticsSlice3Service } from './collection-rate-analytics-slice3.service';
...
describe('CollectionRateAnalyticsSlice3Service', () => {
  let service: CollectionRateAnalyticsSlice3Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new CollectionRateAnalyticsSlice3Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        CollectionRateAnalyticsSlice3Service,
        { provide: getRepositoryToken(CollectionRateAnalyticsSlice3Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(CollectionRateAnalyticsSlice3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CollectionRateAnalyticsSlice3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CollectionRateAnalyticsSlice3.updated' }));
  });
