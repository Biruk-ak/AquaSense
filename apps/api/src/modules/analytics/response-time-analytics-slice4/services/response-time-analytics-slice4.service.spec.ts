 ...
 * Unit tests: ResponseTimeAnalyticsSlice4
 * @copyright Biruk-ak
 */
...
import { ResponseTimeAnalyticsSlice4Service } from './response-time-analytics-slice4.service';
...
describe('ResponseTimeAnalyticsSlice4Service', () => {
  let service: ResponseTimeAnalyticsSlice4Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new ResponseTimeAnalyticsSlice4Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ResponseTimeAnalyticsSlice4Service,
        { provide: getRepositoryToken(ResponseTimeAnalyticsSlice4Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(ResponseTimeAnalyticsSlice4Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ResponseTimeAnalyticsSlice4.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ResponseTimeAnalyticsSlice4.updated' }));
  });
