 ...
 * Unit tests: MeterHealthAnalyticsSlice5
 * @copyright Biruk-ak
 */
...
import { MeterHealthAnalyticsSlice5Service } from './meter-health-analytics-slice5.service';
...
describe('MeterHealthAnalyticsSlice5Service', () => {
  let service: MeterHealthAnalyticsSlice5Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new MeterHealthAnalyticsSlice5Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        MeterHealthAnalyticsSlice5Service,
        { provide: getRepositoryToken(MeterHealthAnalyticsSlice5Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(MeterHealthAnalyticsSlice5Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'MeterHealthAnalyticsSlice5.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'MeterHealthAnalyticsSlice5.updated' }));
  });
