 ...
 * Unit tests: OutageAnalyticsSlice4
 * @copyright Biruk-ak
 */
...
import { OutageAnalyticsSlice4Service } from './outage-analytics-slice4.service';
...
describe('OutageAnalyticsSlice4Service', () => {
  let service: OutageAnalyticsSlice4Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new OutageAnalyticsSlice4Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        OutageAnalyticsSlice4Service,
        { provide: getRepositoryToken(OutageAnalyticsSlice4Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(OutageAnalyticsSlice4Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'OutageAnalyticsSlice4.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'OutageAnalyticsSlice4.updated' }));
  });
