 ...
 * Unit tests: ConsumptionAnalyticsSlice1
 * @copyright Biruk-ak
 */
...
import { ConsumptionAnalyticsSlice1Service } from './consumption-analytics-slice1.service';
...
describe('ConsumptionAnalyticsSlice1Service', () => {
  let service: ConsumptionAnalyticsSlice1Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new ConsumptionAnalyticsSlice1Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ConsumptionAnalyticsSlice1Service,
        { provide: getRepositoryToken(ConsumptionAnalyticsSlice1Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(ConsumptionAnalyticsSlice1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ConsumptionAnalyticsSlice1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ConsumptionAnalyticsSlice1.updated' }));
  });
