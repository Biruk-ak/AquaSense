 ...
 * Unit tests: MeterHealthAnalyticsSlice4
 * @copyright Biruk-ak
 */
...
import { MeterHealthAnalyticsSlice4Service } from './meter-health-analytics-slice4.service';
...
describe('MeterHealthAnalyticsSlice4Service', () => {
  let service: MeterHealthAnalyticsSlice4Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new MeterHealthAnalyticsSlice4Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        MeterHealthAnalyticsSlice4Service,
        { provide: getRepositoryToken(MeterHealthAnalyticsSlice4Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(MeterHealthAnalyticsSlice4Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'MeterHealthAnalyticsSlice4.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'MeterHealthAnalyticsSlice4.updated' }));
  });
