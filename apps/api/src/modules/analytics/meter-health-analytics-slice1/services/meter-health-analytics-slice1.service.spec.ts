 ...
 * Unit tests: MeterHealthAnalyticsSlice1
 * @copyright Biruk-ak
 */
...
import { MeterHealthAnalyticsSlice1Service } from './meter-health-analytics-slice1.service';
...
describe('MeterHealthAnalyticsSlice1Service', () => {
  let service: MeterHealthAnalyticsSlice1Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new MeterHealthAnalyticsSlice1Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        MeterHealthAnalyticsSlice1Service,
        { provide: getRepositoryToken(MeterHealthAnalyticsSlice1Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(MeterHealthAnalyticsSlice1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'MeterHealthAnalyticsSlice1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'MeterHealthAnalyticsSlice1.updated' }));
  });
