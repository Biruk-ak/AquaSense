 ...
 * Unit tests: ResponseTimeAnalyticsSlice1
 * @copyright Biruk-ak
 */
...
import { ResponseTimeAnalyticsSlice1Service } from './response-time-analytics-slice1.service';
...
describe('ResponseTimeAnalyticsSlice1Service', () => {
  let service: ResponseTimeAnalyticsSlice1Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new ResponseTimeAnalyticsSlice1Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ResponseTimeAnalyticsSlice1Service,
        { provide: getRepositoryToken(ResponseTimeAnalyticsSlice1Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(ResponseTimeAnalyticsSlice1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ResponseTimeAnalyticsSlice1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ResponseTimeAnalyticsSlice1.updated' }));
  });
