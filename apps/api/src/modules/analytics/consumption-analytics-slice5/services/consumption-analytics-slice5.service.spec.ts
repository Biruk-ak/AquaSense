 ...
 * Unit tests: ConsumptionAnalyticsSlice5
 * @copyright Biruk-ak
 */
...
import { ConsumptionAnalyticsSlice5Service } from './consumption-analytics-slice5.service';
...
describe('ConsumptionAnalyticsSlice5Service', () => {
  let service: ConsumptionAnalyticsSlice5Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new ConsumptionAnalyticsSlice5Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ConsumptionAnalyticsSlice5Service,
        { provide: getRepositoryToken(ConsumptionAnalyticsSlice5Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(ConsumptionAnalyticsSlice5Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ConsumptionAnalyticsSlice5.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ConsumptionAnalyticsSlice5.updated' }));
  });
