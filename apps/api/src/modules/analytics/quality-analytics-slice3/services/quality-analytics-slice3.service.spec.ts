 ...
 * Unit tests: QualityAnalyticsSlice3
 * @copyright Biruk-ak
 */
...
import { QualityAnalyticsSlice3Service } from './quality-analytics-slice3.service';
...
describe('QualityAnalyticsSlice3Service', () => {
  let service: QualityAnalyticsSlice3Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new QualityAnalyticsSlice3Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        QualityAnalyticsSlice3Service,
        { provide: getRepositoryToken(QualityAnalyticsSlice3Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(QualityAnalyticsSlice3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'QualityAnalyticsSlice3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'QualityAnalyticsSlice3.updated' }));
  });
