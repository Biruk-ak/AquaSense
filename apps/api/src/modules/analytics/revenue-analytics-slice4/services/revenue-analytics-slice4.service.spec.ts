 ...
 * Unit tests: RevenueAnalyticsSlice4
 * @copyright Biruk-ak
 */
...
import { RevenueAnalyticsSlice4Service } from './revenue-analytics-slice4.service';
...
describe('RevenueAnalyticsSlice4Service', () => {
  let service: RevenueAnalyticsSlice4Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new RevenueAnalyticsSlice4Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        RevenueAnalyticsSlice4Service,
        { provide: getRepositoryToken(RevenueAnalyticsSlice4Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(RevenueAnalyticsSlice4Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'RevenueAnalyticsSlice4.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'RevenueAnalyticsSlice4.updated' }));
  });
