...
import { NrwAnalyticsSlice3Service } from './nrw-analytics-slice3.service';
...
describe('NrwAnalyticsSlice3Service', () => {
  let service: NrwAnalyticsSlice3Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new NrwAnalyticsSlice3Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        NrwAnalyticsSlice3Service,
        { provide: getRepositoryToken(NrwAnalyticsSlice3Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(NrwAnalyticsSlice3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'NrwAnalyticsSlice3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'NrwAnalyticsSlice3.updated' }));
  });
