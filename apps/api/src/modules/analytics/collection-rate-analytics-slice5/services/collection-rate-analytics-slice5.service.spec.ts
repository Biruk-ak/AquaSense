 ...
 * Unit tests: CollectionRateAnalyticsSlice5
 * @copyright Biruk-ak
 */
...
import { CollectionRateAnalyticsSlice5Service } from './collection-rate-analytics-slice5.service';
...
describe('CollectionRateAnalyticsSlice5Service', () => {
  let service: CollectionRateAnalyticsSlice5Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new CollectionRateAnalyticsSlice5Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        CollectionRateAnalyticsSlice5Service,
        { provide: getRepositoryToken(CollectionRateAnalyticsSlice5Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(CollectionRateAnalyticsSlice5Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CollectionRateAnalyticsSlice5.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CollectionRateAnalyticsSlice5.updated' }));
  });
