...
import { NrwAnalyticsSlice1Service } from './nrw-analytics-slice1.service';
...
describe('NrwAnalyticsSlice1Service', () => {
  let service: NrwAnalyticsSlice1Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new NrwAnalyticsSlice1Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        NrwAnalyticsSlice1Service,
        { provide: getRepositoryToken(NrwAnalyticsSlice1Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(NrwAnalyticsSlice1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'NrwAnalyticsSlice1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'NrwAnalyticsSlice1.updated' }));
  });
