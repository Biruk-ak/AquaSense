 ...
 * Unit tests: OutageAnalyticsSlice5
 * @copyright Biruk-ak
 */
...
import { OutageAnalyticsSlice5Service } from './outage-analytics-slice5.service';
...
describe('OutageAnalyticsSlice5Service', () => {
  let service: OutageAnalyticsSlice5Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new OutageAnalyticsSlice5Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        OutageAnalyticsSlice5Service,
        { provide: getRepositoryToken(OutageAnalyticsSlice5Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(OutageAnalyticsSlice5Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'OutageAnalyticsSlice5.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'OutageAnalyticsSlice5.updated' }));
  });
