 ...
 * Unit tests: CollectionRateAnalyticsSlice1
 * @copyright Biruk-ak
 */
...
import { CollectionRateAnalyticsSlice1Service } from './collection-rate-analytics-slice1.service';
...
describe('CollectionRateAnalyticsSlice1Service', () => {
  let service: CollectionRateAnalyticsSlice1Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new CollectionRateAnalyticsSlice1Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        CollectionRateAnalyticsSlice1Service,
        { provide: getRepositoryToken(CollectionRateAnalyticsSlice1Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(CollectionRateAnalyticsSlice1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CollectionRateAnalyticsSlice1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CollectionRateAnalyticsSlice1.updated' }));
  });
