 ...
 * Unit tests: RevenueAnalyticsSlice2
 * @copyright Biruk-ak
 */
...
import { RevenueAnalyticsSlice2Service } from './revenue-analytics-slice2.service';
...
describe('RevenueAnalyticsSlice2Service', () => {
  let service: RevenueAnalyticsSlice2Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new RevenueAnalyticsSlice2Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        RevenueAnalyticsSlice2Service,
        { provide: getRepositoryToken(RevenueAnalyticsSlice2Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(RevenueAnalyticsSlice2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'RevenueAnalyticsSlice2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'RevenueAnalyticsSlice2.updated' }));
  });
