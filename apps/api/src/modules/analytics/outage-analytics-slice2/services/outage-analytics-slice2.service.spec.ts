 ...
 * Unit tests: OutageAnalyticsSlice2
 * @copyright Biruk-ak
 */
...
import { OutageAnalyticsSlice2Service } from './outage-analytics-slice2.service';
...
describe('OutageAnalyticsSlice2Service', () => {
  let service: OutageAnalyticsSlice2Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new OutageAnalyticsSlice2Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        OutageAnalyticsSlice2Service,
        { provide: getRepositoryToken(OutageAnalyticsSlice2Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(OutageAnalyticsSlice2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'OutageAnalyticsSlice2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'OutageAnalyticsSlice2.updated' }));
  });
