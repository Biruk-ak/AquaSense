 ...
 * Unit tests: PressureAnalyticsSlice2
 * @copyright Biruk-ak
 */
...
import { PressureAnalyticsSlice2Service } from './pressure-analytics-slice2.service';
...
describe('PressureAnalyticsSlice2Service', () => {
  let service: PressureAnalyticsSlice2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new PressureAnalyticsSlice2Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        PressureAnalyticsSlice2Service,
        { provide: getRepositoryToken(PressureAnalyticsSlice2Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(PressureAnalyticsSlice2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PressureAnalyticsSlice2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PressureAnalyticsSlice2.updated' }));
  });
