...
import { NrwAnalyticsSlice5Service } from './nrw-analytics-slice5.service';
...
describe('NrwAnalyticsSlice5Service', () => {
  let service: NrwAnalyticsSlice5Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new NrwAnalyticsSlice5Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        NrwAnalyticsSlice5Service,
        { provide: getRepositoryToken(NrwAnalyticsSlice5Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(NrwAnalyticsSlice5Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'NrwAnalyticsSlice5.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'NrwAnalyticsSlice5.updated' }));
  });
