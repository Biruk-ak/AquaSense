 ...
 * Unit tests: PressureAnalyticsSlice3
 * @copyright Biruk-ak
 */
...
import { PressureAnalyticsSlice3Service } from './pressure-analytics-slice3.service';
...
describe('PressureAnalyticsSlice3Service', () => {
  let service: PressureAnalyticsSlice3Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new PressureAnalyticsSlice3Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        PressureAnalyticsSlice3Service,
        { provide: getRepositoryToken(PressureAnalyticsSlice3Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(PressureAnalyticsSlice3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PressureAnalyticsSlice3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PressureAnalyticsSlice3.updated' }));
  });
