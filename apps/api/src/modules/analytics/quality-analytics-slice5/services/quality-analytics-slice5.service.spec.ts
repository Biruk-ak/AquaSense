 ...
 * Unit tests: QualityAnalyticsSlice5
 * @copyright Biruk-ak
 */
...
import { QualityAnalyticsSlice5Service } from './quality-analytics-slice5.service';
...
describe('QualityAnalyticsSlice5Service', () => {
  let service: QualityAnalyticsSlice5Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new QualityAnalyticsSlice5Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        QualityAnalyticsSlice5Service,
        { provide: getRepositoryToken(QualityAnalyticsSlice5Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(QualityAnalyticsSlice5Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'QualityAnalyticsSlice5.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'QualityAnalyticsSlice5.updated' }));
  });
