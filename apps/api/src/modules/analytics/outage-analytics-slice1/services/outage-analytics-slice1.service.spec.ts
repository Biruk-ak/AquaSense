 ...
 * Unit tests: OutageAnalyticsSlice1
 * @copyright Biruk-ak
 */
...
import { OutageAnalyticsSlice1Service } from './outage-analytics-slice1.service';
...
describe('OutageAnalyticsSlice1Service', () => {
  let service: OutageAnalyticsSlice1Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new OutageAnalyticsSlice1Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        OutageAnalyticsSlice1Service,
        { provide: getRepositoryToken(OutageAnalyticsSlice1Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(OutageAnalyticsSlice1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'OutageAnalyticsSlice1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'OutageAnalyticsSlice1.updated' }));
  });
