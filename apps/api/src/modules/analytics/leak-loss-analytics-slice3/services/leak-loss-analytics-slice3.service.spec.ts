 ...
 * Unit tests: LeakLossAnalyticsSlice3
 * @copyright Biruk-ak
 */
...
import { LeakLossAnalyticsSlice3Service } from './leak-loss-analytics-slice3.service';
...
describe('LeakLossAnalyticsSlice3Service', () => {
  let service: LeakLossAnalyticsSlice3Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new LeakLossAnalyticsSlice3Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        LeakLossAnalyticsSlice3Service,
        { provide: getRepositoryToken(LeakLossAnalyticsSlice3Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(LeakLossAnalyticsSlice3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakLossAnalyticsSlice3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakLossAnalyticsSlice3.updated' }));
  });
