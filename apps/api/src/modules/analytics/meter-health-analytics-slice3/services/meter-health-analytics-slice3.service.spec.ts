 ...
 * Unit tests: MeterHealthAnalyticsSlice3
 * @copyright Biruk-ak
 */
...
import { MeterHealthAnalyticsSlice3Service } from './meter-health-analytics-slice3.service';
...
describe('MeterHealthAnalyticsSlice3Service', () => {
  let service: MeterHealthAnalyticsSlice3Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new MeterHealthAnalyticsSlice3Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        MeterHealthAnalyticsSlice3Service,
        { provide: getRepositoryToken(MeterHealthAnalyticsSlice3Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(MeterHealthAnalyticsSlice3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'MeterHealthAnalyticsSlice3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'MeterHealthAnalyticsSlice3.updated' }));
  });
