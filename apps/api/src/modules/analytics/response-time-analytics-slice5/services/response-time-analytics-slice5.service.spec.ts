 ...
 * Unit tests: ResponseTimeAnalyticsSlice5
 * @copyright Biruk-ak
 */
...
import { ResponseTimeAnalyticsSlice5Service } from './response-time-analytics-slice5.service';
...
describe('ResponseTimeAnalyticsSlice5Service', () => {
  let service: ResponseTimeAnalyticsSlice5Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new ResponseTimeAnalyticsSlice5Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ResponseTimeAnalyticsSlice5Service,
        { provide: getRepositoryToken(ResponseTimeAnalyticsSlice5Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(ResponseTimeAnalyticsSlice5Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ResponseTimeAnalyticsSlice5.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ResponseTimeAnalyticsSlice5.updated' }));
  });
