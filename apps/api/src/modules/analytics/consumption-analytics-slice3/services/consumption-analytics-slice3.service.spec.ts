 ...
 * Unit tests: ConsumptionAnalyticsSlice3
 * @copyright Biruk-ak
 */
...
import { ConsumptionAnalyticsSlice3Service } from './consumption-analytics-slice3.service';
...
describe('ConsumptionAnalyticsSlice3Service', () => {
  let service: ConsumptionAnalyticsSlice3Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new ConsumptionAnalyticsSlice3Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ConsumptionAnalyticsSlice3Service,
        { provide: getRepositoryToken(ConsumptionAnalyticsSlice3Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(ConsumptionAnalyticsSlice3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ConsumptionAnalyticsSlice3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ConsumptionAnalyticsSlice3.updated' }));
  });
