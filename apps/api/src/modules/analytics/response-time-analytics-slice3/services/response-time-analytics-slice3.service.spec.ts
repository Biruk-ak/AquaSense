 ...
 * Unit tests: ResponseTimeAnalyticsSlice3
 * @copyright Biruk-ak
 */
...
import { ResponseTimeAnalyticsSlice3Service } from './response-time-analytics-slice3.service';
...
describe('ResponseTimeAnalyticsSlice3Service', () => {
  let service: ResponseTimeAnalyticsSlice3Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new ResponseTimeAnalyticsSlice3Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ResponseTimeAnalyticsSlice3Service,
        { provide: getRepositoryToken(ResponseTimeAnalyticsSlice3Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(ResponseTimeAnalyticsSlice3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ResponseTimeAnalyticsSlice3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ResponseTimeAnalyticsSlice3.updated' }));
  });
