 ...
 * Unit tests: ConsumptionAnalyticsSlice4
 * @copyright Biruk-ak
 */
...
import { ConsumptionAnalyticsSlice4Service } from './consumption-analytics-slice4.service';
...
describe('ConsumptionAnalyticsSlice4Service', () => {
  let service: ConsumptionAnalyticsSlice4Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new ConsumptionAnalyticsSlice4Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ConsumptionAnalyticsSlice4Service,
        { provide: getRepositoryToken(ConsumptionAnalyticsSlice4Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(ConsumptionAnalyticsSlice4Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ConsumptionAnalyticsSlice4.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ConsumptionAnalyticsSlice4.updated' }));
  });
