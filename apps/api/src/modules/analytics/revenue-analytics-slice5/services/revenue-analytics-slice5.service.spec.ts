 ...
 * Unit tests: RevenueAnalyticsSlice5
 * @copyright Biruk-ak
 */
...
import { RevenueAnalyticsSlice5Service } from './revenue-analytics-slice5.service';
...
describe('RevenueAnalyticsSlice5Service', () => {
  let service: RevenueAnalyticsSlice5Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new RevenueAnalyticsSlice5Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        RevenueAnalyticsSlice5Service,
        { provide: getRepositoryToken(RevenueAnalyticsSlice5Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(RevenueAnalyticsSlice5Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'RevenueAnalyticsSlice5.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'RevenueAnalyticsSlice5.updated' }));
  });
