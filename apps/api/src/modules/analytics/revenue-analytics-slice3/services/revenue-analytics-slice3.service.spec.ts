 ...
 * Unit tests: RevenueAnalyticsSlice3
 * @copyright Biruk-ak
 */
...
import { RevenueAnalyticsSlice3Service } from './revenue-analytics-slice3.service';
...
describe('RevenueAnalyticsSlice3Service', () => {
  let service: RevenueAnalyticsSlice3Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new RevenueAnalyticsSlice3Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        RevenueAnalyticsSlice3Service,
        { provide: getRepositoryToken(RevenueAnalyticsSlice3Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(RevenueAnalyticsSlice3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'RevenueAnalyticsSlice3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'RevenueAnalyticsSlice3.updated' }));
  });
