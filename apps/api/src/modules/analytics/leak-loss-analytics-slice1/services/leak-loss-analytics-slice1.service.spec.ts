 ...
 * Unit tests: LeakLossAnalyticsSlice1
 * @copyright Biruk-ak
 */
...
import { LeakLossAnalyticsSlice1Service } from './leak-loss-analytics-slice1.service';
...
describe('LeakLossAnalyticsSlice1Service', () => {
  let service: LeakLossAnalyticsSlice1Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new LeakLossAnalyticsSlice1Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        LeakLossAnalyticsSlice1Service,
        { provide: getRepositoryToken(LeakLossAnalyticsSlice1Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(LeakLossAnalyticsSlice1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakLossAnalyticsSlice1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakLossAnalyticsSlice1.updated' }));
  });
