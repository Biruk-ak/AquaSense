 ...
 * Unit tests: LeakLossAnalyticsSlice4
 * @copyright Biruk-ak
 */
...
import { LeakLossAnalyticsSlice4Service } from './leak-loss-analytics-slice4.service';
...
describe('LeakLossAnalyticsSlice4Service', () => {
  let service: LeakLossAnalyticsSlice4Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new LeakLossAnalyticsSlice4Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        LeakLossAnalyticsSlice4Service,
        { provide: getRepositoryToken(LeakLossAnalyticsSlice4Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(LeakLossAnalyticsSlice4Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakLossAnalyticsSlice4.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakLossAnalyticsSlice4.updated' }));
  });
