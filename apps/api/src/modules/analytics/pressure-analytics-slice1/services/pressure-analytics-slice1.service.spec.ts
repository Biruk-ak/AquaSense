 ...
 * Unit tests: PressureAnalyticsSlice1
 * @copyright Biruk-ak
 */
...
import { PressureAnalyticsSlice1Service } from './pressure-analytics-slice1.service';
...
describe('PressureAnalyticsSlice1Service', () => {
  let service: PressureAnalyticsSlice1Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new PressureAnalyticsSlice1Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        PressureAnalyticsSlice1Service,
        { provide: getRepositoryToken(PressureAnalyticsSlice1Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(PressureAnalyticsSlice1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PressureAnalyticsSlice1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PressureAnalyticsSlice1.updated' }));
  });
