 ...
 * Unit tests: ResponseTimeAnalyticsSlice2
 * @copyright Biruk-ak
 */
...
import { ResponseTimeAnalyticsSlice2Service } from './response-time-analytics-slice2.service';
...
describe('ResponseTimeAnalyticsSlice2Service', () => {
  let service: ResponseTimeAnalyticsSlice2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new ResponseTimeAnalyticsSlice2Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ResponseTimeAnalyticsSlice2Service,
        { provide: getRepositoryToken(ResponseTimeAnalyticsSlice2Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(ResponseTimeAnalyticsSlice2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ResponseTimeAnalyticsSlice2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ResponseTimeAnalyticsSlice2.updated' }));
  });
