 ...
 * Unit tests: PressureAnalyticsSlice5
 * @copyright Biruk-ak
 */
...
import { PressureAnalyticsSlice5Service } from './pressure-analytics-slice5.service';
...
describe('PressureAnalyticsSlice5Service', () => {
  let service: PressureAnalyticsSlice5Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new PressureAnalyticsSlice5Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        PressureAnalyticsSlice5Service,
        { provide: getRepositoryToken(PressureAnalyticsSlice5Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(PressureAnalyticsSlice5Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PressureAnalyticsSlice5.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PressureAnalyticsSlice5.updated' }));
  });
