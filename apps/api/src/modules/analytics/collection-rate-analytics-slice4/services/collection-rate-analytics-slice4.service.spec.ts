 ...
 * Unit tests: CollectionRateAnalyticsSlice4
 * @copyright Biruk-ak
 */
...
import { CollectionRateAnalyticsSlice4Service } from './collection-rate-analytics-slice4.service';
...
describe('CollectionRateAnalyticsSlice4Service', () => {
  let service: CollectionRateAnalyticsSlice4Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new CollectionRateAnalyticsSlice4Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        CollectionRateAnalyticsSlice4Service,
        { provide: getRepositoryToken(CollectionRateAnalyticsSlice4Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(CollectionRateAnalyticsSlice4Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CollectionRateAnalyticsSlice4.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CollectionRateAnalyticsSlice4.updated' }));
  });
