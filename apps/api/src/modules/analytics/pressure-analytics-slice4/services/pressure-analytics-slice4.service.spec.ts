 ...
 * Unit tests: PressureAnalyticsSlice4
 * @copyright Biruk-ak
 */
...
import { PressureAnalyticsSlice4Service } from './pressure-analytics-slice4.service';
...
describe('PressureAnalyticsSlice4Service', () => {
  let service: PressureAnalyticsSlice4Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new PressureAnalyticsSlice4Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        PressureAnalyticsSlice4Service,
        { provide: getRepositoryToken(PressureAnalyticsSlice4Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(PressureAnalyticsSlice4Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PressureAnalyticsSlice4.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PressureAnalyticsSlice4.updated' }));
  });
