 ...
 * Unit tests: QualityAnalyticsSlice1
 * @copyright Biruk-ak
 */
...
import { QualityAnalyticsSlice1Service } from './quality-analytics-slice1.service';
...
describe('QualityAnalyticsSlice1Service', () => {
  let service: QualityAnalyticsSlice1Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new QualityAnalyticsSlice1Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        QualityAnalyticsSlice1Service,
        { provide: getRepositoryToken(QualityAnalyticsSlice1Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(QualityAnalyticsSlice1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'QualityAnalyticsSlice1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'QualityAnalyticsSlice1.updated' }));
  });
