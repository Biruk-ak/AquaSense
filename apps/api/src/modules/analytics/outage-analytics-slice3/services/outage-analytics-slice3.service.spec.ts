 ...
 * Unit tests: OutageAnalyticsSlice3
 * @copyright Biruk-ak
 */
...
import { OutageAnalyticsSlice3Service } from './outage-analytics-slice3.service';
...
describe('OutageAnalyticsSlice3Service', () => {
  let service: OutageAnalyticsSlice3Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new OutageAnalyticsSlice3Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        OutageAnalyticsSlice3Service,
        { provide: getRepositoryToken(OutageAnalyticsSlice3Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(OutageAnalyticsSlice3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'OutageAnalyticsSlice3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'OutageAnalyticsSlice3.updated' }));
  });
