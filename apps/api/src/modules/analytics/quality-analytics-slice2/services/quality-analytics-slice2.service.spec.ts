 ...
 * Unit tests: QualityAnalyticsSlice2
 * @copyright Biruk-ak
 */
...
import { QualityAnalyticsSlice2Service } from './quality-analytics-slice2.service';
...
describe('QualityAnalyticsSlice2Service', () => {
  let service: QualityAnalyticsSlice2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new QualityAnalyticsSlice2Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        QualityAnalyticsSlice2Service,
        { provide: getRepositoryToken(QualityAnalyticsSlice2Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(QualityAnalyticsSlice2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'QualityAnalyticsSlice2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'QualityAnalyticsSlice2.updated' }));
  });
