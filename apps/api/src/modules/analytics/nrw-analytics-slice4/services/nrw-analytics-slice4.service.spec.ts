...
import { NrwAnalyticsSlice4Service } from './nrw-analytics-slice4.service';
...
describe('NrwAnalyticsSlice4Service', () => {
  let service: NrwAnalyticsSlice4Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new NrwAnalyticsSlice4Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        NrwAnalyticsSlice4Service,
        { provide: getRepositoryToken(NrwAnalyticsSlice4Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(NrwAnalyticsSlice4Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'NrwAnalyticsSlice4.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'NrwAnalyticsSlice4.updated' }));
  });
