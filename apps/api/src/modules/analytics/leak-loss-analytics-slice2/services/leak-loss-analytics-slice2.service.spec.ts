 ...
 * Unit tests: LeakLossAnalyticsSlice2
 * @copyright Biruk-ak
 */
...
import { LeakLossAnalyticsSlice2Service } from './leak-loss-analytics-slice2.service';
...
describe('LeakLossAnalyticsSlice2Service', () => {
  let service: LeakLossAnalyticsSlice2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new LeakLossAnalyticsSlice2Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        LeakLossAnalyticsSlice2Service,
        { provide: getRepositoryToken(LeakLossAnalyticsSlice2Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(LeakLossAnalyticsSlice2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakLossAnalyticsSlice2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakLossAnalyticsSlice2.updated' }));
  });
