 ...
 * Unit tests: MeterHealthAnalyticsSlice2
 * @copyright Biruk-ak
 */
...
import { MeterHealthAnalyticsSlice2Service } from './meter-health-analytics-slice2.service';
...
describe('MeterHealthAnalyticsSlice2Service', () => {
  let service: MeterHealthAnalyticsSlice2Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new MeterHealthAnalyticsSlice2Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        MeterHealthAnalyticsSlice2Service,
        { provide: getRepositoryToken(MeterHealthAnalyticsSlice2Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(MeterHealthAnalyticsSlice2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'MeterHealthAnalyticsSlice2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'MeterHealthAnalyticsSlice2.updated' }));
  });
