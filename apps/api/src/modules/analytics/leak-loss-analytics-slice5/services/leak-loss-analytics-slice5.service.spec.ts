 ...
 * Unit tests: LeakLossAnalyticsSlice5
 * @copyright Biruk-ak
 */
...
import { LeakLossAnalyticsSlice5Service } from './leak-loss-analytics-slice5.service';
...
describe('LeakLossAnalyticsSlice5Service', () => {
  let service: LeakLossAnalyticsSlice5Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new LeakLossAnalyticsSlice5Snapshot(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        LeakLossAnalyticsSlice5Service,
        { provide: getRepositoryToken(LeakLossAnalyticsSlice5Snapshot), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(LeakLossAnalyticsSlice5Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakLossAnalyticsSlice5.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakLossAnalyticsSlice5.updated' }));
  });
