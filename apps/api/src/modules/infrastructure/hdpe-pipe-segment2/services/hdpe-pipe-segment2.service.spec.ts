...
import { HDPEPipeSegment2Service } from './hdpe-pipe-segment2.service';
...
describe('HDPEPipeSegment2Service', () => {
  let service: HDPEPipeSegment2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new HDPEPipeSegment2Asset(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        HDPEPipeSegment2Service,
        { provide: getRepositoryToken(HDPEPipeSegment2Asset), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(HDPEPipeSegment2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'HDPEPipeSegment2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'HDPEPipeSegment2.updated' }));
  });
