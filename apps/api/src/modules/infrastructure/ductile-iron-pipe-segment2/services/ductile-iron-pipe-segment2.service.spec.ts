 ...
 * Unit tests: DuctileIronPipeSegment2
 * @copyright Biruk-ak
 */
...
import { DuctileIronPipeSegment2Service } from './ductile-iron-pipe-segment2.service';
...
describe('DuctileIronPipeSegment2Service', () => {
  let service: DuctileIronPipeSegment2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new DuctileIronPipeSegment2Asset(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        DuctileIronPipeSegment2Service,
        { provide: getRepositoryToken(DuctileIronPipeSegment2Asset), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(DuctileIronPipeSegment2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'DuctileIronPipeSegment2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'DuctileIronPipeSegment2.updated' }));
  });
