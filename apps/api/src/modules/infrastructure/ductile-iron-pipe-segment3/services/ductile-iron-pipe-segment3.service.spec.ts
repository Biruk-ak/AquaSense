 ...
 * Unit tests: DuctileIronPipeSegment3
 * @copyright Biruk-ak
 */
...
import { DuctileIronPipeSegment3Service } from './ductile-iron-pipe-segment3.service';
...
describe('DuctileIronPipeSegment3Service', () => {
  let service: DuctileIronPipeSegment3Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new DuctileIronPipeSegment3Asset(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        DuctileIronPipeSegment3Service,
        { provide: getRepositoryToken(DuctileIronPipeSegment3Asset), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(DuctileIronPipeSegment3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'DuctileIronPipeSegment3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'DuctileIronPipeSegment3.updated' }));
  });
