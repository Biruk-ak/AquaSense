 ...
 * Unit tests: TreatmentPlantAsset1
 * @copyright Biruk-ak
 */
...
import { TreatmentPlantAsset1Service } from './treatment-plant-asset1.service';
...
describe('TreatmentPlantAsset1Service', () => {
  let service: TreatmentPlantAsset1Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new TreatmentPlantAsset1Record(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        TreatmentPlantAsset1Service,
        { provide: getRepositoryToken(TreatmentPlantAsset1Record), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(TreatmentPlantAsset1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'TreatmentPlantAsset1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'TreatmentPlantAsset1.updated' }));
  });
