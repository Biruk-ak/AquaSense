...
import { TankAsset1Service } from './tank-asset1.service';
import { TankAsset1Record } from '../entities/tank-asset1-record.entity';
import { AuditService, EventBusService } from '@aquasense/common';

describe('TankAsset1Service', () => {
  let service: TankAsset1Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new TankAsset1Record(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        TankAsset1Service,
        { provide: getRepositoryToken(TankAsset1Record), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(TankAsset1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'TankAsset1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'TankAsset1.updated' }));
  });
