...
import { HydrantAsset2Service } from './hydrant-asset2.service';
...
describe('HydrantAsset2Service', () => {
  let service: HydrantAsset2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new HydrantAsset2Record(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        HydrantAsset2Service,
        { provide: getRepositoryToken(HydrantAsset2Record), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(HydrantAsset2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'HydrantAsset2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'HydrantAsset2.updated' }));
  });
