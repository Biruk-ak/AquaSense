...
import { ReservoirAsset1Service } from './reservoir-asset1.service';
...
describe('ReservoirAsset1Service', () => {
  let service: ReservoirAsset1Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new ReservoirAsset1Record(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ReservoirAsset1Service,
        { provide: getRepositoryToken(ReservoirAsset1Record), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(ReservoirAsset1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ReservoirAsset1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ReservoirAsset1.updated' }));
  });
