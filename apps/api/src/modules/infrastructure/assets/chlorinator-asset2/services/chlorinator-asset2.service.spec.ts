...
import { ChlorinatorAsset2Service } from './chlorinator-asset2.service';
...
describe('ChlorinatorAsset2Service', () => {
  let service: ChlorinatorAsset2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new ChlorinatorAsset2Record(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ChlorinatorAsset2Service,
        { provide: getRepositoryToken(ChlorinatorAsset2Record), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(ChlorinatorAsset2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ChlorinatorAsset2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ChlorinatorAsset2.updated' }));
  });
