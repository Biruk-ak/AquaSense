...
import { PumpStationAsset2Service } from './pump-station-asset2.service';
...
describe('PumpStationAsset2Service', () => {
  let service: PumpStationAsset2Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new PumpStationAsset2Record(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        PumpStationAsset2Service,
        { provide: getRepositoryToken(PumpStationAsset2Record), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(PumpStationAsset2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PumpStationAsset2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PumpStationAsset2.updated' }));
  });
