 ...
 * Unit tests: TreatmentPlantAsset2
 * @copyright Biruk-ak
 */
...
import { TreatmentPlantAsset2Service } from './treatment-plant-asset2.service';
...
describe('TreatmentPlantAsset2Service', () => {
  let service: TreatmentPlantAsset2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new TreatmentPlantAsset2Record(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        TreatmentPlantAsset2Service,
        { provide: getRepositoryToken(TreatmentPlantAsset2Record), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(TreatmentPlantAsset2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'TreatmentPlantAsset2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'TreatmentPlantAsset2.updated' }));
  });
