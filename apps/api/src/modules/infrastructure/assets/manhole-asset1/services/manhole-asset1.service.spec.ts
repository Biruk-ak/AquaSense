...
import { ManholeAsset1Service } from './manhole-asset1.service';
...
describe('ManholeAsset1Service', () => {
  let service: ManholeAsset1Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new ManholeAsset1Record(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ManholeAsset1Service,
        { provide: getRepositoryToken(ManholeAsset1Record), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(ManholeAsset1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ManholeAsset1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ManholeAsset1.updated' }));
  });
