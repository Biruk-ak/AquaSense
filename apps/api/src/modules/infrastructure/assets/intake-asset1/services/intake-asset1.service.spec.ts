...
import { IntakeAsset1Service } from './intake-asset1.service';
...
describe('IntakeAsset1Service', () => {
  let service: IntakeAsset1Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new IntakeAsset1Record(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        IntakeAsset1Service,
        { provide: getRepositoryToken(IntakeAsset1Record), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(IntakeAsset1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'IntakeAsset1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'IntakeAsset1.updated' }));
  });
