...
import { HydrantAsset1Service } from './hydrant-asset1.service';
...
describe('HydrantAsset1Service', () => {
  let service: HydrantAsset1Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new HydrantAsset1Record(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        HydrantAsset1Service,
        { provide: getRepositoryToken(HydrantAsset1Record), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(HydrantAsset1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'HydrantAsset1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'HydrantAsset1.updated' }));
  });
