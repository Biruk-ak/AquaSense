...
import { ManholeAsset2Service } from './manhole-asset2.service';
...
describe('ManholeAsset2Service', () => {
  let service: ManholeAsset2Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new ManholeAsset2Record(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ManholeAsset2Service,
        { provide: getRepositoryToken(ManholeAsset2Record), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(ManholeAsset2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ManholeAsset2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ManholeAsset2.updated' }));
  });
