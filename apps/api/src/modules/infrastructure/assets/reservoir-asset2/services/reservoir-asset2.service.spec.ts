...
import { ReservoirAsset2Service } from './reservoir-asset2.service';
...
describe('ReservoirAsset2Service', () => {
  let service: ReservoirAsset2Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new ReservoirAsset2Record(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ReservoirAsset2Service,
        { provide: getRepositoryToken(ReservoirAsset2Record), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(ReservoirAsset2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ReservoirAsset2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ReservoirAsset2.updated' }));
  });
