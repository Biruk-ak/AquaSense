...
import { IntakeAsset2Service } from './intake-asset2.service';
...
describe('IntakeAsset2Service', () => {
  let service: IntakeAsset2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new IntakeAsset2Record(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        IntakeAsset2Service,
        { provide: getRepositoryToken(IntakeAsset2Record), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(IntakeAsset2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'IntakeAsset2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'IntakeAsset2.updated' }));
  });
