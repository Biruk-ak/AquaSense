...
import { BoosterAsset1Service } from './booster-asset1.service';
...
describe('BoosterAsset1Service', () => {
  let service: BoosterAsset1Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new BoosterAsset1Record(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        BoosterAsset1Service,
        { provide: getRepositoryToken(BoosterAsset1Record), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(BoosterAsset1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'BoosterAsset1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'BoosterAsset1.updated' }));
  });
