...
import { PumpStationAsset1Service } from './pump-station-asset1.service';
...
describe('PumpStationAsset1Service', () => {
  let service: PumpStationAsset1Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new PumpStationAsset1Record(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        PumpStationAsset1Service,
        { provide: getRepositoryToken(PumpStationAsset1Record), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(PumpStationAsset1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PumpStationAsset1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PumpStationAsset1.updated' }));
  });
