...
import { FilterAsset1Service } from './filter-asset1.service';
...
describe('FilterAsset1Service', () => {
  let service: FilterAsset1Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new FilterAsset1Record(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        FilterAsset1Service,
        { provide: getRepositoryToken(FilterAsset1Record), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(FilterAsset1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'FilterAsset1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'FilterAsset1.updated' }));
  });
