...
import { TankAsset2Service } from './tank-asset2.service';
import { TankAsset2Record } from '../entities/tank-asset2-record.entity';
import { AuditService, EventBusService } from '@aquasense/common';

describe('TankAsset2Service', () => {
  let service: TankAsset2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new TankAsset2Record(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        TankAsset2Service,
        { provide: getRepositoryToken(TankAsset2Record), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(TankAsset2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'TankAsset2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'TankAsset2.updated' }));
  });
