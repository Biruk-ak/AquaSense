...
import { WellAsset1Service } from './well-asset1.service';
import { WellAsset1Record } from '../entities/well-asset1-record.entity';
import { AuditService, EventBusService } from '@aquasense/common';

describe('WellAsset1Service', () => {
  let service: WellAsset1Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new WellAsset1Record(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        WellAsset1Service,
        { provide: getRepositoryToken(WellAsset1Record), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(WellAsset1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'WellAsset1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'WellAsset1.updated' }));
  });
