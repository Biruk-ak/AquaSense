...
import { ChlorinatorAsset1Service } from './chlorinator-asset1.service';
...
describe('ChlorinatorAsset1Service', () => {
  let service: ChlorinatorAsset1Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new ChlorinatorAsset1Record(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ChlorinatorAsset1Service,
        { provide: getRepositoryToken(ChlorinatorAsset1Record), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(ChlorinatorAsset1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ChlorinatorAsset1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ChlorinatorAsset1.updated' }));
  });
