...
import { WellAsset2Service } from './well-asset2.service';
import { WellAsset2Record } from '../entities/well-asset2-record.entity';
import { AuditService, EventBusService } from '@aquasense/common';

describe('WellAsset2Service', () => {
  let service: WellAsset2Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new WellAsset2Record(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        WellAsset2Service,
        { provide: getRepositoryToken(WellAsset2Record), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(WellAsset2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'WellAsset2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'WellAsset2.updated' }));
  });
