...
import { BoosterAsset2Service } from './booster-asset2.service';
...
describe('BoosterAsset2Service', () => {
  let service: BoosterAsset2Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new BoosterAsset2Record(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        BoosterAsset2Service,
        { provide: getRepositoryToken(BoosterAsset2Record), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(BoosterAsset2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'BoosterAsset2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'BoosterAsset2.updated' }));
  });
