 ...
 * Unit tests: CastIronPipeSegment1
 * @copyright Biruk-ak
 */
...
import { CastIronPipeSegment1Service } from './cast-iron-pipe-segment1.service';
...
describe('CastIronPipeSegment1Service', () => {
  let service: CastIronPipeSegment1Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new CastIronPipeSegment1Asset(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        CastIronPipeSegment1Service,
        { provide: getRepositoryToken(CastIronPipeSegment1Asset), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(CastIronPipeSegment1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CastIronPipeSegment1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CastIronPipeSegment1.updated' }));
  });
