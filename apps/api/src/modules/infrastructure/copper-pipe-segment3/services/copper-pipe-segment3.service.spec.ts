...
import { CopperPipeSegment3Service } from './copper-pipe-segment3.service';
...
describe('CopperPipeSegment3Service', () => {
  let service: CopperPipeSegment3Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new CopperPipeSegment3Asset(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        CopperPipeSegment3Service,
        { provide: getRepositoryToken(CopperPipeSegment3Asset), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(CopperPipeSegment3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CopperPipeSegment3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CopperPipeSegment3.updated' }));
  });
