...
import { SteelPipeSegment3Service } from './steel-pipe-segment3.service';
...
describe('SteelPipeSegment3Service', () => {
  let service: SteelPipeSegment3Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new SteelPipeSegment3Asset(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        SteelPipeSegment3Service,
        { provide: getRepositoryToken(SteelPipeSegment3Asset), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(SteelPipeSegment3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'SteelPipeSegment3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'SteelPipeSegment3.updated' }));
  });
