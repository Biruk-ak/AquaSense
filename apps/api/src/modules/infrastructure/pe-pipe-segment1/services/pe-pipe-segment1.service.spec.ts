...
import { PEPipeSegment1Service } from './pe-pipe-segment1.service';
...
describe('PEPipeSegment1Service', () => {
  let service: PEPipeSegment1Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new PEPipeSegment1Asset(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        PEPipeSegment1Service,
        { provide: getRepositoryToken(PEPipeSegment1Asset), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(PEPipeSegment1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PEPipeSegment1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PEPipeSegment1.updated' }));
  });
