...
import { PVCPipeSegment2Service } from './pvc-pipe-segment2.service';
...
describe('PVCPipeSegment2Service', () => {
  let service: PVCPipeSegment2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new PVCPipeSegment2Asset(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        PVCPipeSegment2Service,
        { provide: getRepositoryToken(PVCPipeSegment2Asset), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(PVCPipeSegment2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PVCPipeSegment2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PVCPipeSegment2.updated' }));
  });
