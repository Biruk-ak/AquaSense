...
import { PVCPipeSegment3Service } from './pvc-pipe-segment3.service';
...
describe('PVCPipeSegment3Service', () => {
  let service: PVCPipeSegment3Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new PVCPipeSegment3Asset(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        PVCPipeSegment3Service,
        { provide: getRepositoryToken(PVCPipeSegment3Asset), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(PVCPipeSegment3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PVCPipeSegment3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PVCPipeSegment3.updated' }));
  });
