 ...
 * Unit tests: CastIronPipeSegment2
 * @copyright Biruk-ak
 */
...
import { CastIronPipeSegment2Service } from './cast-iron-pipe-segment2.service';
...
describe('CastIronPipeSegment2Service', () => {
  let service: CastIronPipeSegment2Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new CastIronPipeSegment2Asset(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        CastIronPipeSegment2Service,
        { provide: getRepositoryToken(CastIronPipeSegment2Asset), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(CastIronPipeSegment2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CastIronPipeSegment2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CastIronPipeSegment2.updated' }));
  });
