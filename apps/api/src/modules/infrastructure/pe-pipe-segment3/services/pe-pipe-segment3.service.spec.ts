...
import { PEPipeSegment3Service } from './pe-pipe-segment3.service';
...
describe('PEPipeSegment3Service', () => {
  let service: PEPipeSegment3Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new PEPipeSegment3Asset(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        PEPipeSegment3Service,
        { provide: getRepositoryToken(PEPipeSegment3Asset), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(PEPipeSegment3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PEPipeSegment3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PEPipeSegment3.updated' }));
  });
