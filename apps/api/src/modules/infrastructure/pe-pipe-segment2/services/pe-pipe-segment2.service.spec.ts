...
import { PEPipeSegment2Service } from './pe-pipe-segment2.service';
...
describe('PEPipeSegment2Service', () => {
  let service: PEPipeSegment2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new PEPipeSegment2Asset(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        PEPipeSegment2Service,
        { provide: getRepositoryToken(PEPipeSegment2Asset), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(PEPipeSegment2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PEPipeSegment2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PEPipeSegment2.updated' }));
  });
