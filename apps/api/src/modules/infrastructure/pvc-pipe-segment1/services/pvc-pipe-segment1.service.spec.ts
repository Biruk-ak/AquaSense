...
import { PVCPipeSegment1Service } from './pvc-pipe-segment1.service';
...
describe('PVCPipeSegment1Service', () => {
  let service: PVCPipeSegment1Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new PVCPipeSegment1Asset(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        PVCPipeSegment1Service,
        { provide: getRepositoryToken(PVCPipeSegment1Asset), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(PVCPipeSegment1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PVCPipeSegment1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PVCPipeSegment1.updated' }));
  });
