...
import { HDPEPipeSegment3Service } from './hdpe-pipe-segment3.service';
...
describe('HDPEPipeSegment3Service', () => {
  let service: HDPEPipeSegment3Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new HDPEPipeSegment3Asset(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        HDPEPipeSegment3Service,
        { provide: getRepositoryToken(HDPEPipeSegment3Asset), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(HDPEPipeSegment3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'HDPEPipeSegment3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'HDPEPipeSegment3.updated' }));
  });
