 ...
 * Unit tests: DuctileIronPipeSegment1
 * @copyright Biruk-ak
 */
...
import { DuctileIronPipeSegment1Service } from './ductile-iron-pipe-segment1.service';
...
describe('DuctileIronPipeSegment1Service', () => {
  let service: DuctileIronPipeSegment1Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new DuctileIronPipeSegment1Asset(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        DuctileIronPipeSegment1Service,
        { provide: getRepositoryToken(DuctileIronPipeSegment1Asset), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(DuctileIronPipeSegment1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'DuctileIronPipeSegment1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'DuctileIronPipeSegment1.updated' }));
  });
