...
import { SteelPipeSegment2Service } from './steel-pipe-segment2.service';
...
describe('SteelPipeSegment2Service', () => {
  let service: SteelPipeSegment2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new SteelPipeSegment2Asset(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        SteelPipeSegment2Service,
        { provide: getRepositoryToken(SteelPipeSegment2Asset), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(SteelPipeSegment2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'SteelPipeSegment2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'SteelPipeSegment2.updated' }));
  });
