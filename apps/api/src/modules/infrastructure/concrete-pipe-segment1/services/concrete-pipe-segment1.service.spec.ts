 ...
 * Unit tests: ConcretePipeSegment1
 * @copyright Biruk-ak
 */
...
import { ConcretePipeSegment1Service } from './concrete-pipe-segment1.service';
...
describe('ConcretePipeSegment1Service', () => {
  let service: ConcretePipeSegment1Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new ConcretePipeSegment1Asset(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ConcretePipeSegment1Service,
        { provide: getRepositoryToken(ConcretePipeSegment1Asset), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(ConcretePipeSegment1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ConcretePipeSegment1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ConcretePipeSegment1.updated' }));
  });
