...
import { CopperPipeSegment2Service } from './copper-pipe-segment2.service';
...
describe('CopperPipeSegment2Service', () => {
  let service: CopperPipeSegment2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new CopperPipeSegment2Asset(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        CopperPipeSegment2Service,
        { provide: getRepositoryToken(CopperPipeSegment2Asset), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(CopperPipeSegment2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CopperPipeSegment2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CopperPipeSegment2.updated' }));
  });
