 ...
 * Unit tests: ConcretePipeSegment2
 * @copyright Biruk-ak
 */
...
import { ConcretePipeSegment2Service } from './concrete-pipe-segment2.service';
...
describe('ConcretePipeSegment2Service', () => {
  let service: ConcretePipeSegment2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new ConcretePipeSegment2Asset(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ConcretePipeSegment2Service,
        { provide: getRepositoryToken(ConcretePipeSegment2Asset), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(ConcretePipeSegment2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ConcretePipeSegment2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ConcretePipeSegment2.updated' }));
  });
