...
import { CopperPipeSegment1Service } from './copper-pipe-segment1.service';
...
describe('CopperPipeSegment1Service', () => {
  let service: CopperPipeSegment1Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new CopperPipeSegment1Asset(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        CopperPipeSegment1Service,
        { provide: getRepositoryToken(CopperPipeSegment1Asset), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(CopperPipeSegment1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CopperPipeSegment1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CopperPipeSegment1.updated' }));
  });
