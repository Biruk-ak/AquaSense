 ...
 * Unit tests: ConcretePipeSegment3
 * @copyright Biruk-ak
 */
...
import { ConcretePipeSegment3Service } from './concrete-pipe-segment3.service';
...
describe('ConcretePipeSegment3Service', () => {
  let service: ConcretePipeSegment3Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new ConcretePipeSegment3Asset(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ConcretePipeSegment3Service,
        { provide: getRepositoryToken(ConcretePipeSegment3Asset), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(ConcretePipeSegment3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ConcretePipeSegment3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ConcretePipeSegment3.updated' }));
  });
