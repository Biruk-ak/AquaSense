...
import { HDPEPipeSegment1Service } from './hdpe-pipe-segment1.service';
...
describe('HDPEPipeSegment1Service', () => {
  let service: HDPEPipeSegment1Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new HDPEPipeSegment1Asset(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        HDPEPipeSegment1Service,
        { provide: getRepositoryToken(HDPEPipeSegment1Asset), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(HDPEPipeSegment1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'HDPEPipeSegment1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'HDPEPipeSegment1.updated' }));
  });
