 ...
 * Unit tests: CastIronPipeSegment3
 * @copyright Biruk-ak
 */
...
import { CastIronPipeSegment3Service } from './cast-iron-pipe-segment3.service';
...
describe('CastIronPipeSegment3Service', () => {
  let service: CastIronPipeSegment3Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new CastIronPipeSegment3Asset(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        CastIronPipeSegment3Service,
        { provide: getRepositoryToken(CastIronPipeSegment3Asset), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(CastIronPipeSegment3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CastIronPipeSegment3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CastIronPipeSegment3.updated' }));
  });
