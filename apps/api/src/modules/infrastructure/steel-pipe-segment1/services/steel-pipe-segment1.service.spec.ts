...
import { SteelPipeSegment1Service } from './steel-pipe-segment1.service';
...
describe('SteelPipeSegment1Service', () => {
  let service: SteelPipeSegment1Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new SteelPipeSegment1Asset(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        SteelPipeSegment1Service,
        { provide: getRepositoryToken(SteelPipeSegment1Asset), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(SteelPipeSegment1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'SteelPipeSegment1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'SteelPipeSegment1.updated' }));
  });
