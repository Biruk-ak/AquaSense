...
import { ValleyDistrictService } from './valley-district.service';
...
describe('ValleyDistrictService', () => {
  let service: ValleyDistrictService;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new ValleyDistrictProfile(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ValleyDistrictService,
        { provide: getRepositoryToken(ValleyDistrictProfile), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(ValleyDistrictService);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ValleyDistrict.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ValleyDistrict.updated' }));
  });
