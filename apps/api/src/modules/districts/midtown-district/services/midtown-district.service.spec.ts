...
import { MidtownDistrictService } from './midtown-district.service';
...
describe('MidtownDistrictService', () => {
  let service: MidtownDistrictService;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new MidtownDistrictProfile(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        MidtownDistrictService,
        { provide: getRepositoryToken(MidtownDistrictProfile), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(MidtownDistrictService);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'MidtownDistrict.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'MidtownDistrict.updated' }));
  });
