...
import { SouthDistrictService } from './south-district.service';
...
describe('SouthDistrictService', () => {
  let service: SouthDistrictService;
  const repo = {
    create: jest.fn((x) => Object.assign(new SouthDistrictProfile(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        SouthDistrictService,
        { provide: getRepositoryToken(SouthDistrictProfile), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(SouthDistrictService);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'SouthDistrict.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'SouthDistrict.updated' }));
  });
