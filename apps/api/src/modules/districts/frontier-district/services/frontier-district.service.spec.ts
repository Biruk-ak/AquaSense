...
import { FrontierDistrictService } from './frontier-district.service';
...
describe('FrontierDistrictService', () => {
  let service: FrontierDistrictService;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new FrontierDistrictProfile(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        FrontierDistrictService,
        { provide: getRepositoryToken(FrontierDistrictProfile), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(FrontierDistrictService);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'FrontierDistrict.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'FrontierDistrict.updated' }));
  });
