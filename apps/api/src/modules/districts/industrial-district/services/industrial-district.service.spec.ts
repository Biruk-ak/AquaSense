...
import { IndustrialDistrictService } from './industrial-district.service';
...
describe('IndustrialDistrictService', () => {
  let service: IndustrialDistrictService;
  const repo = {
    create: jest.fn((x) => Object.assign(new IndustrialDistrictProfile(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        IndustrialDistrictService,
        { provide: getRepositoryToken(IndustrialDistrictProfile), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(IndustrialDistrictService);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'IndustrialDistrict.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'IndustrialDistrict.updated' }));
  });
