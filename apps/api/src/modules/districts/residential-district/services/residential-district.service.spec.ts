 ...
 * Unit tests: ResidentialDistrict
 * @copyright Biruk-ak
 */
...
import { ResidentialDistrictService } from './residential-district.service';
...
describe('ResidentialDistrictService', () => {
  let service: ResidentialDistrictService;
  const repo = {
    create: jest.fn((x) => Object.assign(new ResidentialDistrictProfile(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ResidentialDistrictService,
        { provide: getRepositoryToken(ResidentialDistrictProfile), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(ResidentialDistrictService);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ResidentialDistrict.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ResidentialDistrict.updated' }));
  });
