...
import { LakesideDistrictService } from './lakeside-district.service';
...
describe('LakesideDistrictService', () => {
  let service: LakesideDistrictService;
  const repo = {
    create: jest.fn((x) => Object.assign(new LakesideDistrictProfile(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        LakesideDistrictService,
        { provide: getRepositoryToken(LakesideDistrictProfile), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(LakesideDistrictService);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LakesideDistrict.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LakesideDistrict.updated' }));
  });
