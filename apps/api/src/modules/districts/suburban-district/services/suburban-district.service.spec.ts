...
import { SuburbanDistrictService } from './suburban-district.service';
...
describe('SuburbanDistrictService', () => {
  let service: SuburbanDistrictService;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new SuburbanDistrictProfile(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        SuburbanDistrictService,
        { provide: getRepositoryToken(SuburbanDistrictProfile), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(SuburbanDistrictService);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'SuburbanDistrict.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'SuburbanDistrict.updated' }));
  });
