...
import { RiversideDistrictService } from './riverside-district.service';
...
describe('RiversideDistrictService', () => {
  let service: RiversideDistrictService;
  const repo = {
    create: jest.fn((x) => Object.assign(new RiversideDistrictProfile(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        RiversideDistrictService,
        { provide: getRepositoryToken(RiversideDistrictProfile), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(RiversideDistrictService);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'RiversideDistrict.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'RiversideDistrict.updated' }));
  });
