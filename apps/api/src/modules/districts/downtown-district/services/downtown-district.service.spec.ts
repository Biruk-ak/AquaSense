...
import { DowntownDistrictService } from './downtown-district.service';
...
describe('DowntownDistrictService', () => {
  let service: DowntownDistrictService;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new DowntownDistrictProfile(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        DowntownDistrictService,
        { provide: getRepositoryToken(DowntownDistrictProfile), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(DowntownDistrictService);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'DowntownDistrict.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'DowntownDistrict.updated' }));
  });
