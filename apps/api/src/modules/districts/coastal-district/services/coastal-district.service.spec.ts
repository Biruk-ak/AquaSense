...
import { CoastalDistrictService } from './coastal-district.service';
...
describe('CoastalDistrictService', () => {
  let service: CoastalDistrictService;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new CoastalDistrictProfile(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        CoastalDistrictService,
        { provide: getRepositoryToken(CoastalDistrictProfile), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(CoastalDistrictService);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CoastalDistrict.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CoastalDistrict.updated' }));
  });
