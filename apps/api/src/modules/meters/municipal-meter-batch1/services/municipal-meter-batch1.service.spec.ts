 ...
 * Unit tests: MunicipalMeterBatch1
 * @copyright Biruk-ak
 */
...
import { MunicipalMeterBatch1Service } from './municipal-meter-batch1.service';
...
describe('MunicipalMeterBatch1Service', () => {
  let service: MunicipalMeterBatch1Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new MunicipalMeterBatch1Reading(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        MunicipalMeterBatch1Service,
        { provide: getRepositoryToken(MunicipalMeterBatch1Reading), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(MunicipalMeterBatch1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'MunicipalMeterBatch1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'MunicipalMeterBatch1.updated' }));
  });
