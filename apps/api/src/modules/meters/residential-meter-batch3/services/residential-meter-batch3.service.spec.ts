 ...
 * Unit tests: ResidentialMeterBatch3
 * @copyright Biruk-ak
 */
...
import { ResidentialMeterBatch3Service } from './residential-meter-batch3.service';
...
describe('ResidentialMeterBatch3Service', () => {
  let service: ResidentialMeterBatch3Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new ResidentialMeterBatch3Reading(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ResidentialMeterBatch3Service,
        { provide: getRepositoryToken(ResidentialMeterBatch3Reading), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(ResidentialMeterBatch3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ResidentialMeterBatch3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ResidentialMeterBatch3.updated' }));
  });
