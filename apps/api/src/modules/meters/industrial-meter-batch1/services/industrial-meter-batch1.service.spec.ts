 ...
 * Unit tests: IndustrialMeterBatch1
 * @copyright Biruk-ak
 */
...
import { IndustrialMeterBatch1Service } from './industrial-meter-batch1.service';
...
describe('IndustrialMeterBatch1Service', () => {
  let service: IndustrialMeterBatch1Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new IndustrialMeterBatch1Reading(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        IndustrialMeterBatch1Service,
        { provide: getRepositoryToken(IndustrialMeterBatch1Reading), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(IndustrialMeterBatch1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'IndustrialMeterBatch1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'IndustrialMeterBatch1.updated' }));
  });
