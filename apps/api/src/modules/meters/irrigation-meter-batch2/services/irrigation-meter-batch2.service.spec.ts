 ...
 * Unit tests: IrrigationMeterBatch2
 * @copyright Biruk-ak
 */
...
import { IrrigationMeterBatch2Service } from './irrigation-meter-batch2.service';
...
describe('IrrigationMeterBatch2Service', () => {
  let service: IrrigationMeterBatch2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new IrrigationMeterBatch2Reading(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        IrrigationMeterBatch2Service,
        { provide: getRepositoryToken(IrrigationMeterBatch2Reading), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(IrrigationMeterBatch2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'IrrigationMeterBatch2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'IrrigationMeterBatch2.updated' }));
  });
