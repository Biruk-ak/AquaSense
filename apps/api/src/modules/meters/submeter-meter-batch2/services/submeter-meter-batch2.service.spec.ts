 ...
 * Unit tests: SubmeterMeterBatch2
 * @copyright Biruk-ak
 */
...
import { SubmeterMeterBatch2Service } from './submeter-meter-batch2.service';
...
describe('SubmeterMeterBatch2Service', () => {
  let service: SubmeterMeterBatch2Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new SubmeterMeterBatch2Reading(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        SubmeterMeterBatch2Service,
        { provide: getRepositoryToken(SubmeterMeterBatch2Reading), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(SubmeterMeterBatch2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'SubmeterMeterBatch2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'SubmeterMeterBatch2.updated' }));
  });
