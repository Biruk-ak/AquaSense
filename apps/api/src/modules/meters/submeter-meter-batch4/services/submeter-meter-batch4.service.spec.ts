 ...
 * Unit tests: SubmeterMeterBatch4
 * @copyright Biruk-ak
 */
...
import { SubmeterMeterBatch4Service } from './submeter-meter-batch4.service';
...
describe('SubmeterMeterBatch4Service', () => {
  let service: SubmeterMeterBatch4Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new SubmeterMeterBatch4Reading(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        SubmeterMeterBatch4Service,
        { provide: getRepositoryToken(SubmeterMeterBatch4Reading), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(SubmeterMeterBatch4Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'SubmeterMeterBatch4.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'SubmeterMeterBatch4.updated' }));
  });
