 ...
 * Unit tests: AgriculturalMeterBatch4
 * @copyright Biruk-ak
 */
...
import { AgriculturalMeterBatch4Service } from './agricultural-meter-batch4.service';
...
describe('AgriculturalMeterBatch4Service', () => {
  let service: AgriculturalMeterBatch4Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new AgriculturalMeterBatch4Reading(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        AgriculturalMeterBatch4Service,
        { provide: getRepositoryToken(AgriculturalMeterBatch4Reading), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(AgriculturalMeterBatch4Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'AgriculturalMeterBatch4.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'AgriculturalMeterBatch4.updated' }));
  });
