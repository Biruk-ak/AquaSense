 ...
 * Unit tests: SubmeterMeterBatch3
 * @copyright Biruk-ak
 */
...
import { SubmeterMeterBatch3Service } from './submeter-meter-batch3.service';
...
describe('SubmeterMeterBatch3Service', () => {
  let service: SubmeterMeterBatch3Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new SubmeterMeterBatch3Reading(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        SubmeterMeterBatch3Service,
        { provide: getRepositoryToken(SubmeterMeterBatch3Reading), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(SubmeterMeterBatch3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'SubmeterMeterBatch3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'SubmeterMeterBatch3.updated' }));
  });
