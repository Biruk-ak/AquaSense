 ...
 * Unit tests: MunicipalMeterBatch4
 * @copyright Biruk-ak
 */
...
import { MunicipalMeterBatch4Service } from './municipal-meter-batch4.service';
...
describe('MunicipalMeterBatch4Service', () => {
  let service: MunicipalMeterBatch4Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new MunicipalMeterBatch4Reading(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        MunicipalMeterBatch4Service,
        { provide: getRepositoryToken(MunicipalMeterBatch4Reading), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(MunicipalMeterBatch4Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'MunicipalMeterBatch4.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'MunicipalMeterBatch4.updated' }));
  });
