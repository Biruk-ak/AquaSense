 ...
 * Unit tests: AgriculturalMeterBatch1
 * @copyright Biruk-ak
 */
...
import { AgriculturalMeterBatch1Service } from './agricultural-meter-batch1.service';
...
describe('AgriculturalMeterBatch1Service', () => {
  let service: AgriculturalMeterBatch1Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new AgriculturalMeterBatch1Reading(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        AgriculturalMeterBatch1Service,
        { provide: getRepositoryToken(AgriculturalMeterBatch1Reading), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(AgriculturalMeterBatch1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'AgriculturalMeterBatch1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'AgriculturalMeterBatch1.updated' }));
  });
