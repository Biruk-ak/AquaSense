 ...
 * Unit tests: CommercialMeterBatch1
 * @copyright Biruk-ak
 */
...
import { CommercialMeterBatch1Service } from './commercial-meter-batch1.service';
...
describe('CommercialMeterBatch1Service', () => {
  let service: CommercialMeterBatch1Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new CommercialMeterBatch1Reading(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        CommercialMeterBatch1Service,
        { provide: getRepositoryToken(CommercialMeterBatch1Reading), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(CommercialMeterBatch1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CommercialMeterBatch1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CommercialMeterBatch1.updated' }));
  });
