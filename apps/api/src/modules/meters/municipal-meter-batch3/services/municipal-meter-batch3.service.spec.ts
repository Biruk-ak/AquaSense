 ...
 * Unit tests: MunicipalMeterBatch3
 * @copyright Biruk-ak
 */
...
import { MunicipalMeterBatch3Service } from './municipal-meter-batch3.service';
...
describe('MunicipalMeterBatch3Service', () => {
  let service: MunicipalMeterBatch3Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new MunicipalMeterBatch3Reading(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        MunicipalMeterBatch3Service,
        { provide: getRepositoryToken(MunicipalMeterBatch3Reading), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(MunicipalMeterBatch3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'MunicipalMeterBatch3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'MunicipalMeterBatch3.updated' }));
  });
