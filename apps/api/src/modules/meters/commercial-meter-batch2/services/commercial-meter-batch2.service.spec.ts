 ...
 * Unit tests: CommercialMeterBatch2
 * @copyright Biruk-ak
 */
...
import { CommercialMeterBatch2Service } from './commercial-meter-batch2.service';
...
describe('CommercialMeterBatch2Service', () => {
  let service: CommercialMeterBatch2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new CommercialMeterBatch2Reading(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        CommercialMeterBatch2Service,
        { provide: getRepositoryToken(CommercialMeterBatch2Reading), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(CommercialMeterBatch2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CommercialMeterBatch2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CommercialMeterBatch2.updated' }));
  });
