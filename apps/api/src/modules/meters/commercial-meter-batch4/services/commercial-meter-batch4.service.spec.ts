 ...
 * Unit tests: CommercialMeterBatch4
 * @copyright Biruk-ak
 */
...
import { CommercialMeterBatch4Service } from './commercial-meter-batch4.service';
...
describe('CommercialMeterBatch4Service', () => {
  let service: CommercialMeterBatch4Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new CommercialMeterBatch4Reading(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        CommercialMeterBatch4Service,
        { provide: getRepositoryToken(CommercialMeterBatch4Reading), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(CommercialMeterBatch4Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CommercialMeterBatch4.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CommercialMeterBatch4.updated' }));
  });
