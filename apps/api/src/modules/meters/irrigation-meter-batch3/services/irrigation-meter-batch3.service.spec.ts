 ...
 * Unit tests: IrrigationMeterBatch3
 * @copyright Biruk-ak
 */
...
import { IrrigationMeterBatch3Service } from './irrigation-meter-batch3.service';
...
describe('IrrigationMeterBatch3Service', () => {
  let service: IrrigationMeterBatch3Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new IrrigationMeterBatch3Reading(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        IrrigationMeterBatch3Service,
        { provide: getRepositoryToken(IrrigationMeterBatch3Reading), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(IrrigationMeterBatch3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'IrrigationMeterBatch3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'IrrigationMeterBatch3.updated' }));
  });
