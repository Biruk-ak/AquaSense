 ...
 * Unit tests: SubmeterMeterBatch1
 * @copyright Biruk-ak
 */
...
import { SubmeterMeterBatch1Service } from './submeter-meter-batch1.service';
...
describe('SubmeterMeterBatch1Service', () => {
  let service: SubmeterMeterBatch1Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new SubmeterMeterBatch1Reading(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        SubmeterMeterBatch1Service,
        { provide: getRepositoryToken(SubmeterMeterBatch1Reading), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(SubmeterMeterBatch1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'SubmeterMeterBatch1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'SubmeterMeterBatch1.updated' }));
  });
