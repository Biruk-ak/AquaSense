 ...
 * Unit tests: IrrigationMeterBatch4
 * @copyright Biruk-ak
 */
...
import { IrrigationMeterBatch4Service } from './irrigation-meter-batch4.service';
...
describe('IrrigationMeterBatch4Service', () => {
  let service: IrrigationMeterBatch4Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new IrrigationMeterBatch4Reading(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        IrrigationMeterBatch4Service,
        { provide: getRepositoryToken(IrrigationMeterBatch4Reading), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(IrrigationMeterBatch4Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'IrrigationMeterBatch4.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'IrrigationMeterBatch4.updated' }));
  });
