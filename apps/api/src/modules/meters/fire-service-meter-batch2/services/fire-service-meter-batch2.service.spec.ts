 ...
 * Unit tests: FireServiceMeterBatch2
 * @copyright Biruk-ak
 */
...
import { FireServiceMeterBatch2Service } from './fire-service-meter-batch2.service';
...
describe('FireServiceMeterBatch2Service', () => {
  let service: FireServiceMeterBatch2Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new FireServiceMeterBatch2Reading(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        FireServiceMeterBatch2Service,
        { provide: getRepositoryToken(FireServiceMeterBatch2Reading), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(FireServiceMeterBatch2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'FireServiceMeterBatch2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'FireServiceMeterBatch2.updated' }));
  });
