 ...
 * Unit tests: ResidentialMeterBatch2
 * @copyright Biruk-ak
 */
...
import { ResidentialMeterBatch2Service } from './residential-meter-batch2.service';
...
describe('ResidentialMeterBatch2Service', () => {
  let service: ResidentialMeterBatch2Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new ResidentialMeterBatch2Reading(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ResidentialMeterBatch2Service,
        { provide: getRepositoryToken(ResidentialMeterBatch2Reading), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(ResidentialMeterBatch2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ResidentialMeterBatch2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ResidentialMeterBatch2.updated' }));
  });
