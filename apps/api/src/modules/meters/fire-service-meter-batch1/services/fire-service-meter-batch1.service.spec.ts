 ...
 * Unit tests: FireServiceMeterBatch1
 * @copyright Biruk-ak
 */
...
import { FireServiceMeterBatch1Service } from './fire-service-meter-batch1.service';
...
describe('FireServiceMeterBatch1Service', () => {
  let service: FireServiceMeterBatch1Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new FireServiceMeterBatch1Reading(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        FireServiceMeterBatch1Service,
        { provide: getRepositoryToken(FireServiceMeterBatch1Reading), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(FireServiceMeterBatch1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'FireServiceMeterBatch1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'FireServiceMeterBatch1.updated' }));
  });
