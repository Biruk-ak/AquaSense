 ...
 * Unit tests: ResidentialMeterBatch4
 * @copyright Biruk-ak
 */
...
import { ResidentialMeterBatch4Service } from './residential-meter-batch4.service';
...
describe('ResidentialMeterBatch4Service', () => {
  let service: ResidentialMeterBatch4Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new ResidentialMeterBatch4Reading(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ResidentialMeterBatch4Service,
        { provide: getRepositoryToken(ResidentialMeterBatch4Reading), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(ResidentialMeterBatch4Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ResidentialMeterBatch4.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ResidentialMeterBatch4.updated' }));
  });
