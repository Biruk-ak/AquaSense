 ...
 * Unit tests: IrrigationMeterBatch1
 * @copyright Biruk-ak
 */
...
import { IrrigationMeterBatch1Service } from './irrigation-meter-batch1.service';
...
describe('IrrigationMeterBatch1Service', () => {
  let service: IrrigationMeterBatch1Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new IrrigationMeterBatch1Reading(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        IrrigationMeterBatch1Service,
        { provide: getRepositoryToken(IrrigationMeterBatch1Reading), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(IrrigationMeterBatch1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'IrrigationMeterBatch1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'IrrigationMeterBatch1.updated' }));
  });
