 ...
 * Unit tests: ResidentialMeterBatch1
 * @copyright Biruk-ak
 */
...
import { ResidentialMeterBatch1Service } from './residential-meter-batch1.service';
...
describe('ResidentialMeterBatch1Service', () => {
  let service: ResidentialMeterBatch1Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new ResidentialMeterBatch1Reading(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ResidentialMeterBatch1Service,
        { provide: getRepositoryToken(ResidentialMeterBatch1Reading), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(ResidentialMeterBatch1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ResidentialMeterBatch1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'ResidentialMeterBatch1.updated' }));
  });
