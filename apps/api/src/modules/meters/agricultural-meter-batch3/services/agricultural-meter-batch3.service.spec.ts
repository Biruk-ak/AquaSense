 ...
 * Unit tests: AgriculturalMeterBatch3
 * @copyright Biruk-ak
 */
...
import { AgriculturalMeterBatch3Service } from './agricultural-meter-batch3.service';
...
describe('AgriculturalMeterBatch3Service', () => {
  let service: AgriculturalMeterBatch3Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new AgriculturalMeterBatch3Reading(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        AgriculturalMeterBatch3Service,
        { provide: getRepositoryToken(AgriculturalMeterBatch3Reading), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(AgriculturalMeterBatch3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'AgriculturalMeterBatch3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'AgriculturalMeterBatch3.updated' }));
  });
