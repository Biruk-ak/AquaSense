 ...
 * Unit tests: CommercialMeterBatch3
 * @copyright Biruk-ak
 */
...
import { CommercialMeterBatch3Service } from './commercial-meter-batch3.service';
...
describe('CommercialMeterBatch3Service', () => {
  let service: CommercialMeterBatch3Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new CommercialMeterBatch3Reading(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        CommercialMeterBatch3Service,
        { provide: getRepositoryToken(CommercialMeterBatch3Reading), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(CommercialMeterBatch3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CommercialMeterBatch3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'CommercialMeterBatch3.updated' }));
  });
