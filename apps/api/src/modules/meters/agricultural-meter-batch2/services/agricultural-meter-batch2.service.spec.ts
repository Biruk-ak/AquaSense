 ...
 * Unit tests: AgriculturalMeterBatch2
 * @copyright Biruk-ak
 */
...
import { AgriculturalMeterBatch2Service } from './agricultural-meter-batch2.service';
...
describe('AgriculturalMeterBatch2Service', () => {
  let service: AgriculturalMeterBatch2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new AgriculturalMeterBatch2Reading(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        AgriculturalMeterBatch2Service,
        { provide: getRepositoryToken(AgriculturalMeterBatch2Reading), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(AgriculturalMeterBatch2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'AgriculturalMeterBatch2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'AgriculturalMeterBatch2.updated' }));
  });
