...
import { MasterMeterBatch4Service } from './master-meter-batch4.service';
...
describe('MasterMeterBatch4Service', () => {
  let service: MasterMeterBatch4Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new MasterMeterBatch4Reading(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        MasterMeterBatch4Service,
        { provide: getRepositoryToken(MasterMeterBatch4Reading), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(MasterMeterBatch4Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'MasterMeterBatch4.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'MasterMeterBatch4.updated' }));
  });
