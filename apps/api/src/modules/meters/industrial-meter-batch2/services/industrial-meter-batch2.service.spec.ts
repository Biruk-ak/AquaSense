 ...
 * Unit tests: IndustrialMeterBatch2
 * @copyright Biruk-ak
 */
...
import { IndustrialMeterBatch2Service } from './industrial-meter-batch2.service';
...
describe('IndustrialMeterBatch2Service', () => {
  let service: IndustrialMeterBatch2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new IndustrialMeterBatch2Reading(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        IndustrialMeterBatch2Service,
        { provide: getRepositoryToken(IndustrialMeterBatch2Reading), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(IndustrialMeterBatch2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'IndustrialMeterBatch2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'IndustrialMeterBatch2.updated' }));
  });
