 ...
 * Unit tests: IndustrialMeterBatch3
 * @copyright Biruk-ak
 */
...
import { IndustrialMeterBatch3Service } from './industrial-meter-batch3.service';
...
describe('IndustrialMeterBatch3Service', () => {
  let service: IndustrialMeterBatch3Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new IndustrialMeterBatch3Reading(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        IndustrialMeterBatch3Service,
        { provide: getRepositoryToken(IndustrialMeterBatch3Reading), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(IndustrialMeterBatch3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'IndustrialMeterBatch3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'IndustrialMeterBatch3.updated' }));
  });
