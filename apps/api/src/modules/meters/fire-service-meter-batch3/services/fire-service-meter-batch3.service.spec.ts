 ...
 * Unit tests: FireServiceMeterBatch3
 * @copyright Biruk-ak
 */
...
import { FireServiceMeterBatch3Service } from './fire-service-meter-batch3.service';
...
describe('FireServiceMeterBatch3Service', () => {
  let service: FireServiceMeterBatch3Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new FireServiceMeterBatch3Reading(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        FireServiceMeterBatch3Service,
        { provide: getRepositoryToken(FireServiceMeterBatch3Reading), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(FireServiceMeterBatch3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'FireServiceMeterBatch3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'FireServiceMeterBatch3.updated' }));
  });
