...
import { MasterMeterBatch1Service } from './master-meter-batch1.service';
...
describe('MasterMeterBatch1Service', () => {
  let service: MasterMeterBatch1Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new MasterMeterBatch1Reading(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        MasterMeterBatch1Service,
        { provide: getRepositoryToken(MasterMeterBatch1Reading), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(MasterMeterBatch1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'MasterMeterBatch1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'MasterMeterBatch1.updated' }));
  });
