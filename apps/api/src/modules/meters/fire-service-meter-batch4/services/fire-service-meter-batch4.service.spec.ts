 ...
 * Unit tests: FireServiceMeterBatch4
 * @copyright Biruk-ak
 */
...
import { FireServiceMeterBatch4Service } from './fire-service-meter-batch4.service';
...
describe('FireServiceMeterBatch4Service', () => {
  let service: FireServiceMeterBatch4Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new FireServiceMeterBatch4Reading(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        FireServiceMeterBatch4Service,
        { provide: getRepositoryToken(FireServiceMeterBatch4Reading), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(FireServiceMeterBatch4Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'FireServiceMeterBatch4.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'FireServiceMeterBatch4.updated' }));
  });
