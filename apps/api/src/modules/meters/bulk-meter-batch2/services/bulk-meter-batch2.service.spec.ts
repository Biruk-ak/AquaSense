...
import { BulkMeterBatch2Service } from './bulk-meter-batch2.service';
...
describe('BulkMeterBatch2Service', () => {
  let service: BulkMeterBatch2Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new BulkMeterBatch2Reading(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        BulkMeterBatch2Service,
        { provide: getRepositoryToken(BulkMeterBatch2Reading), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(BulkMeterBatch2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'BulkMeterBatch2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'BulkMeterBatch2.updated' }));
  });
