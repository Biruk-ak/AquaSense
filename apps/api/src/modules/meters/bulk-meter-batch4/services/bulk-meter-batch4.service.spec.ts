...
import { BulkMeterBatch4Service } from './bulk-meter-batch4.service';
...
describe('BulkMeterBatch4Service', () => {
  let service: BulkMeterBatch4Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new BulkMeterBatch4Reading(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        BulkMeterBatch4Service,
        { provide: getRepositoryToken(BulkMeterBatch4Reading), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(BulkMeterBatch4Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'BulkMeterBatch4.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'BulkMeterBatch4.updated' }));
  });
