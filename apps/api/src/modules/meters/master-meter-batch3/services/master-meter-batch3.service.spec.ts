...
import { MasterMeterBatch3Service } from './master-meter-batch3.service';
...
describe('MasterMeterBatch3Service', () => {
  let service: MasterMeterBatch3Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new MasterMeterBatch3Reading(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        MasterMeterBatch3Service,
        { provide: getRepositoryToken(MasterMeterBatch3Reading), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(MasterMeterBatch3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'MasterMeterBatch3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'MasterMeterBatch3.updated' }));
  });
