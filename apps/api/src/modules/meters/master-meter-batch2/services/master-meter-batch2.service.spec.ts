...
import { MasterMeterBatch2Service } from './master-meter-batch2.service';
...
describe('MasterMeterBatch2Service', () => {
  let service: MasterMeterBatch2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new MasterMeterBatch2Reading(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        MasterMeterBatch2Service,
        { provide: getRepositoryToken(MasterMeterBatch2Reading), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(MasterMeterBatch2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'MasterMeterBatch2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'MasterMeterBatch2.updated' }));
  });
