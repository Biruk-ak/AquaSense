 ...
 * Unit tests: IndustrialMeterBatch4
 * @copyright Biruk-ak
 */
...
import { IndustrialMeterBatch4Service } from './industrial-meter-batch4.service';
...
describe('IndustrialMeterBatch4Service', () => {
  let service: IndustrialMeterBatch4Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new IndustrialMeterBatch4Reading(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        IndustrialMeterBatch4Service,
        { provide: getRepositoryToken(IndustrialMeterBatch4Reading), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(IndustrialMeterBatch4Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'IndustrialMeterBatch4.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'IndustrialMeterBatch4.updated' }));
  });
