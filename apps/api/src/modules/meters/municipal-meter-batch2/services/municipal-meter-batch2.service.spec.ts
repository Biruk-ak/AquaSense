 ...
 * Unit tests: MunicipalMeterBatch2
 * @copyright Biruk-ak
 */
...
import { MunicipalMeterBatch2Service } from './municipal-meter-batch2.service';
...
describe('MunicipalMeterBatch2Service', () => {
  let service: MunicipalMeterBatch2Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new MunicipalMeterBatch2Reading(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        MunicipalMeterBatch2Service,
        { provide: getRepositoryToken(MunicipalMeterBatch2Reading), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(MunicipalMeterBatch2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'MunicipalMeterBatch2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'MunicipalMeterBatch2.updated' }));
  });
