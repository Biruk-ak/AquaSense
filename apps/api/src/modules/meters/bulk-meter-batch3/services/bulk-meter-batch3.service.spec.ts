...
import { BulkMeterBatch3Service } from './bulk-meter-batch3.service';
...
describe('BulkMeterBatch3Service', () => {
  let service: BulkMeterBatch3Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new BulkMeterBatch3Reading(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        BulkMeterBatch3Service,
        { provide: getRepositoryToken(BulkMeterBatch3Reading), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(BulkMeterBatch3Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'BulkMeterBatch3.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'BulkMeterBatch3.updated' }));
  });
