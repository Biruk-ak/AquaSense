...
import { BulkMeterBatch1Service } from './bulk-meter-batch1.service';
...
describe('BulkMeterBatch1Service', () => {
  let service: BulkMeterBatch1Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new BulkMeterBatch1Reading(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        BulkMeterBatch1Service,
        { provide: getRepositoryToken(BulkMeterBatch1Reading), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(BulkMeterBatch1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'BulkMeterBatch1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'BulkMeterBatch1.updated' }));
  });
