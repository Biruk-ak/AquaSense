 ...
 * Unit tests: NewConnectionRequest1
 * @copyright Biruk-ak
 */
...
import { NewConnectionRequest1Service } from './new-connection-request1.service';
...
describe('NewConnectionRequest1Service', () => {
  let service: NewConnectionRequest1Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new NewConnectionRequest1Case(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        NewConnectionRequest1Service,
        { provide: getRepositoryToken(NewConnectionRequest1Case), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(NewConnectionRequest1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'NewConnectionRequest1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'NewConnectionRequest1.updated' }));
  });
