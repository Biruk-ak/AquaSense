 ...
 * Unit tests: BillingDisputeRequest1
 * @copyright Biruk-ak
 */
...
import { BillingDisputeRequest1Service } from './billing-dispute-request1.service';
...
describe('BillingDisputeRequest1Service', () => {
  let service: BillingDisputeRequest1Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new BillingDisputeRequest1Case(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        BillingDisputeRequest1Service,
        { provide: getRepositoryToken(BillingDisputeRequest1Case), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(BillingDisputeRequest1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'BillingDisputeRequest1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'BillingDisputeRequest1.updated' }));
  });
