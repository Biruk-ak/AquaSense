 ...
 * Unit tests: AccountUpdateRequest1
 * @copyright Biruk-ak
 */
...
import { AccountUpdateRequest1Service } from './account-update-request1.service';
...
describe('AccountUpdateRequest1Service', () => {
  let service: AccountUpdateRequest1Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new AccountUpdateRequest1Case(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        AccountUpdateRequest1Service,
        { provide: getRepositoryToken(AccountUpdateRequest1Case), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(AccountUpdateRequest1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'AccountUpdateRequest1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'AccountUpdateRequest1.updated' }));
  });
