...
import { MoveOutRequest2Service } from './move-out-request2.service';
...
describe('MoveOutRequest2Service', () => {
  let service: MoveOutRequest2Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new MoveOutRequest2Case(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        MoveOutRequest2Service,
        { provide: getRepositoryToken(MoveOutRequest2Case), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(MoveOutRequest2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'MoveOutRequest2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'MoveOutRequest2.updated' }));
  });
