...
import { LeakReportRequest1Service } from './leak-report-request1.service';
...
describe('LeakReportRequest1Service', () => {
  let service: LeakReportRequest1Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new LeakReportRequest1Case(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        LeakReportRequest1Service,
        { provide: getRepositoryToken(LeakReportRequest1Case), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(LeakReportRequest1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakReportRequest1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakReportRequest1.updated' }));
  });
