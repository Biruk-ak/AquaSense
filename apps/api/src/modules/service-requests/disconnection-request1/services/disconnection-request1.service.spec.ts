 ...
 * Unit tests: DisconnectionRequest1
 * @copyright Biruk-ak
 */
...
import { DisconnectionRequest1Service } from './disconnection-request1.service';
...
describe('DisconnectionRequest1Service', () => {
  let service: DisconnectionRequest1Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new DisconnectionRequest1Case(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        DisconnectionRequest1Service,
        { provide: getRepositoryToken(DisconnectionRequest1Case), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(DisconnectionRequest1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'DisconnectionRequest1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'DisconnectionRequest1.updated' }));
  });
