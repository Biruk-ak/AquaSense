 ...
 * Unit tests: NewConnectionRequest2
 * @copyright Biruk-ak
 */
...
import { NewConnectionRequest2Service } from './new-connection-request2.service';
...
describe('NewConnectionRequest2Service', () => {
  let service: NewConnectionRequest2Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new NewConnectionRequest2Case(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        NewConnectionRequest2Service,
        { provide: getRepositoryToken(NewConnectionRequest2Case), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(NewConnectionRequest2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'NewConnectionRequest2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'NewConnectionRequest2.updated' }));
  });
