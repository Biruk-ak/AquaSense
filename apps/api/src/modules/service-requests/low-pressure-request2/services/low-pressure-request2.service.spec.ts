 ...
 * Unit tests: LowPressureRequest2
 * @copyright Biruk-ak
 */
...
import { LowPressureRequest2Service } from './low-pressure-request2.service';
...
describe('LowPressureRequest2Service', () => {
  let service: LowPressureRequest2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new LowPressureRequest2Case(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        LowPressureRequest2Service,
        { provide: getRepositoryToken(LowPressureRequest2Case), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(LowPressureRequest2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LowPressureRequest2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LowPressureRequest2.updated' }));
  });
