...
import { HighBillRequest2Service } from './high-bill-request2.service';
...
describe('HighBillRequest2Service', () => {
  let service: HighBillRequest2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new HighBillRequest2Case(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        HighBillRequest2Service,
        { provide: getRepositoryToken(HighBillRequest2Case), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(HighBillRequest2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'HighBillRequest2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'HighBillRequest2.updated' }));
  });
