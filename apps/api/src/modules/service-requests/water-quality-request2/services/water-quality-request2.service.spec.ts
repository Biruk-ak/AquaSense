 ...
 * Unit tests: WaterQualityRequest2
 * @copyright Biruk-ak
 */
...
import { WaterQualityRequest2Service } from './water-quality-request2.service';
...
describe('WaterQualityRequest2Service', () => {
  let service: WaterQualityRequest2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new WaterQualityRequest2Case(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        WaterQualityRequest2Service,
        { provide: getRepositoryToken(WaterQualityRequest2Case), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(WaterQualityRequest2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'WaterQualityRequest2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'WaterQualityRequest2.updated' }));
  });
