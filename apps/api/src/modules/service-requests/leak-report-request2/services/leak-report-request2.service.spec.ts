...
import { LeakReportRequest2Service } from './leak-report-request2.service';
...
describe('LeakReportRequest2Service', () => {
  let service: LeakReportRequest2Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new LeakReportRequest2Case(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        LeakReportRequest2Service,
        { provide: getRepositoryToken(LeakReportRequest2Case), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(LeakReportRequest2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakReportRequest2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LeakReportRequest2.updated' }));
  });
