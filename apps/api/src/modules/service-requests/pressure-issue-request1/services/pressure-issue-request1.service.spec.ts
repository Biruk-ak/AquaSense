 ...
 * Unit tests: PressureIssueRequest1
 * @copyright Biruk-ak
 */
...
import { PressureIssueRequest1Service } from './pressure-issue-request1.service';
...
describe('PressureIssueRequest1Service', () => {
  let service: PressureIssueRequest1Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new PressureIssueRequest1Case(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        PressureIssueRequest1Service,
        { provide: getRepositoryToken(PressureIssueRequest1Case), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(PressureIssueRequest1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PressureIssueRequest1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PressureIssueRequest1.updated' }));
  });
