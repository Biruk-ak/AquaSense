 ...
 * Unit tests: WaterQualityRequest1
 * @copyright Biruk-ak
 */
...
import { WaterQualityRequest1Service } from './water-quality-request1.service';
...
describe('WaterQualityRequest1Service', () => {
  let service: WaterQualityRequest1Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new WaterQualityRequest1Case(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        WaterQualityRequest1Service,
        { provide: getRepositoryToken(WaterQualityRequest1Case), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(WaterQualityRequest1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'WaterQualityRequest1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'WaterQualityRequest1.updated' }));
  });
