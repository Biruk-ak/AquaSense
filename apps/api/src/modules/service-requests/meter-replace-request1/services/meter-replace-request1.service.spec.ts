 ...
 * Unit tests: MeterReplaceRequest1
 * @copyright Biruk-ak
 */
...
import { MeterReplaceRequest1Service } from './meter-replace-request1.service';
...
describe('MeterReplaceRequest1Service', () => {
  let service: MeterReplaceRequest1Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new MeterReplaceRequest1Case(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        MeterReplaceRequest1Service,
        { provide: getRepositoryToken(MeterReplaceRequest1Case), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(MeterReplaceRequest1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'MeterReplaceRequest1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'MeterReplaceRequest1.updated' }));
  });
