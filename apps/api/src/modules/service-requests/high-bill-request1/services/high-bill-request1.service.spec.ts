...
import { HighBillRequest1Service } from './high-bill-request1.service';
...
describe('HighBillRequest1Service', () => {
  let service: HighBillRequest1Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new HighBillRequest1Case(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        HighBillRequest1Service,
        { provide: getRepositoryToken(HighBillRequest1Case), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(HighBillRequest1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'HighBillRequest1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'HighBillRequest1.updated' }));
  });
