...
import { NoWaterRequest1Service } from './no-water-request1.service';
...
describe('NoWaterRequest1Service', () => {
  let service: NoWaterRequest1Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new NoWaterRequest1Case(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        NoWaterRequest1Service,
        { provide: getRepositoryToken(NoWaterRequest1Case), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(NoWaterRequest1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'NoWaterRequest1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'NoWaterRequest1.updated' }));
  });
