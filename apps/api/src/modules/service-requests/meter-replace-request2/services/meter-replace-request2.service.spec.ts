 ...
 * Unit tests: MeterReplaceRequest2
 * @copyright Biruk-ak
 */
...
import { MeterReplaceRequest2Service } from './meter-replace-request2.service';
...
describe('MeterReplaceRequest2Service', () => {
  let service: MeterReplaceRequest2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new MeterReplaceRequest2Case(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        MeterReplaceRequest2Service,
        { provide: getRepositoryToken(MeterReplaceRequest2Case), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(MeterReplaceRequest2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'MeterReplaceRequest2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'MeterReplaceRequest2.updated' }));
  });
