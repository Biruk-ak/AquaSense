 ...
 * Unit tests: PressureIssueRequest2
 * @copyright Biruk-ak
 */
...
import { PressureIssueRequest2Service } from './pressure-issue-request2.service';
...
describe('PressureIssueRequest2Service', () => {
  let service: PressureIssueRequest2Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new PressureIssueRequest2Case(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        PressureIssueRequest2Service,
        { provide: getRepositoryToken(PressureIssueRequest2Case), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(PressureIssueRequest2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PressureIssueRequest2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PressureIssueRequest2.updated' }));
  });
