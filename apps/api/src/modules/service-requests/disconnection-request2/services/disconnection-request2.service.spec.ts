 ...
 * Unit tests: DisconnectionRequest2
 * @copyright Biruk-ak
 */
...
import { DisconnectionRequest2Service } from './disconnection-request2.service';
...
describe('DisconnectionRequest2Service', () => {
  let service: DisconnectionRequest2Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new DisconnectionRequest2Case(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        DisconnectionRequest2Service,
        { provide: getRepositoryToken(DisconnectionRequest2Case), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(DisconnectionRequest2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'DisconnectionRequest2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'DisconnectionRequest2.updated' }));
  });
