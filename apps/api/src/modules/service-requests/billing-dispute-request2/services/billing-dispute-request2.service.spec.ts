 ...
 * Unit tests: BillingDisputeRequest2
 * @copyright Biruk-ak
 */
...
import { BillingDisputeRequest2Service } from './billing-dispute-request2.service';
...
describe('BillingDisputeRequest2Service', () => {
  let service: BillingDisputeRequest2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new BillingDisputeRequest2Case(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        BillingDisputeRequest2Service,
        { provide: getRepositoryToken(BillingDisputeRequest2Case), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(BillingDisputeRequest2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'BillingDisputeRequest2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'BillingDisputeRequest2.updated' }));
  });
