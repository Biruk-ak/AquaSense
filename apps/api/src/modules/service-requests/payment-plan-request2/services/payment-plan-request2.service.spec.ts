 ...
 * Unit tests: PaymentPlanRequest2
 * @copyright Biruk-ak
 */
...
import { PaymentPlanRequest2Service } from './payment-plan-request2.service';
...
describe('PaymentPlanRequest2Service', () => {
  let service: PaymentPlanRequest2Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new PaymentPlanRequest2Case(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        PaymentPlanRequest2Service,
        { provide: getRepositoryToken(PaymentPlanRequest2Case), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(PaymentPlanRequest2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PaymentPlanRequest2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PaymentPlanRequest2.updated' }));
  });
