 ...
 * Unit tests: AccountUpdateRequest2
 * @copyright Biruk-ak
 */
...
import { AccountUpdateRequest2Service } from './account-update-request2.service';
...
describe('AccountUpdateRequest2Service', () => {
  let service: AccountUpdateRequest2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new AccountUpdateRequest2Case(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        AccountUpdateRequest2Service,
        { provide: getRepositoryToken(AccountUpdateRequest2Case), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(AccountUpdateRequest2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'AccountUpdateRequest2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'AccountUpdateRequest2.updated' }));
  });
