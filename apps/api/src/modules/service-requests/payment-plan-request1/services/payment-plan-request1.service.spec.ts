 ...
 * Unit tests: PaymentPlanRequest1
 * @copyright Biruk-ak
 */
...
import { PaymentPlanRequest1Service } from './payment-plan-request1.service';
...
describe('PaymentPlanRequest1Service', () => {
  let service: PaymentPlanRequest1Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new PaymentPlanRequest1Case(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        PaymentPlanRequest1Service,
        { provide: getRepositoryToken(PaymentPlanRequest1Case), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(PaymentPlanRequest1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PaymentPlanRequest1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'PaymentPlanRequest1.updated' }));
  });
