...
import { MoveInRequest1Service } from './move-in-request1.service';
...
describe('MoveInRequest1Service', () => {
  let service: MoveInRequest1Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new MoveInRequest1Case(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        MoveInRequest1Service,
        { provide: getRepositoryToken(MoveInRequest1Case), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(MoveInRequest1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'MoveInRequest1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'MoveInRequest1.updated' }));
  });
