...
import { InspectionRequest2Service } from './inspection-request2.service';
...
describe('InspectionRequest2Service', () => {
  let service: InspectionRequest2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new InspectionRequest2Case(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        InspectionRequest2Service,
        { provide: getRepositoryToken(InspectionRequest2Case), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(InspectionRequest2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'InspectionRequest2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'InspectionRequest2.updated' }));
  });
