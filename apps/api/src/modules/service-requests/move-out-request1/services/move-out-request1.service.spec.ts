...
import { MoveOutRequest1Service } from './move-out-request1.service';
...
describe('MoveOutRequest1Service', () => {
  let service: MoveOutRequest1Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new MoveOutRequest1Case(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        MoveOutRequest1Service,
        { provide: getRepositoryToken(MoveOutRequest1Case), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(MoveOutRequest1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'MoveOutRequest1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'MoveOutRequest1.updated' }));
  });
