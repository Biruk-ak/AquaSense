 ...
 * Unit tests: LowPressureRequest1
 * @copyright Biruk-ak
 */
...
import { LowPressureRequest1Service } from './low-pressure-request1.service';
...
describe('LowPressureRequest1Service', () => {
  let service: LowPressureRequest1Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new LowPressureRequest1Case(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        LowPressureRequest1Service,
        { provide: getRepositoryToken(LowPressureRequest1Case), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(LowPressureRequest1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LowPressureRequest1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'LowPressureRequest1.updated' }));
  });
