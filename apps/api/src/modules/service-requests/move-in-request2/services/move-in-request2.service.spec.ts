...
import { MoveInRequest2Service } from './move-in-request2.service';
...
describe('MoveInRequest2Service', () => {
  let service: MoveInRequest2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new MoveInRequest2Case(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        MoveInRequest2Service,
        { provide: getRepositoryToken(MoveInRequest2Case), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(MoveInRequest2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'MoveInRequest2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'MoveInRequest2.updated' }));
  });
