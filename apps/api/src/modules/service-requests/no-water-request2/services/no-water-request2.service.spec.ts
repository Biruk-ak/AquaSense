...
import { NoWaterRequest2Service } from './no-water-request2.service';
...
describe('NoWaterRequest2Service', () => {
  let service: NoWaterRequest2Service;
  const repo = {
    create: jest.fn((x) => Object.assign(new NoWaterRequest2Case(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        NoWaterRequest2Service,
        { provide: getRepositoryToken(NoWaterRequest2Case), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(NoWaterRequest2Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'NoWaterRequest2.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'NoWaterRequest2.updated' }));
  });
