...
import { InspectionRequest1Service } from './inspection-request1.service';
...
describe('InspectionRequest1Service', () => {
  let service: InspectionRequest1Service;
  const repo: any = {
    create: jest.fn((x) => Object.assign(new InspectionRequest1Case(), x, {
      validateInvariants: () => [],
      toPublicView: () => ({ ...x }),
      id: 'test-id-001',
    })),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        InspectionRequest1Service,
        { provide: getRepositoryToken(InspectionRequest1Case), useValue: repo },
        { provide: AuditService, useValue: audit },
        { provide: EventBusService, useValue: eventBus },
      ],
    }).compile();
    service = module.get(InspectionRequest1Service);
  });
  ...
  it('creates a record and audits the action', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'InspectionRequest1.created' }));
  ...
  it('updates an existing record', async () => {
    ...
    expect(audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: 'InspectionRequest1.updated' }));
  });
